...
依赖方向严格向下流动。领域 crate 不可依赖 dream-core-app，
dream-core-common 没有任何内部依赖。

## Crate 层级

项目采用 Cargo workspace 组织，共 33 个 crate，分为五层。这 33 个 crate 实际是两条独立的命名血统：

- **`dream-core-*`**（26 个）——产品核心：会话、渠道、团队协作、文件、Office、MCP 等，个人版/开源版就是这一层。
- **`dream-domain-*`**（7 个，原 `one-*`）——企业版/商业化层：组织租户、数字员工、计费、SSO、企业组织维度、部署预留、需求协作看板。**不属于原上游血统**，是后来独立新增的一整族，依赖方向上叠在 `dream-core-*` 之上（见下方"企业层"）。
...
| Crate | 职责 |
|-------|------|
| `dream-core-common` | 共享错误类型（ApiError）、枚举、ID 生成、加密工具、时间戳、分页 |
| `dream-core-api-types` | 所有 HTTP/WebSocket 的请求和响应类型，是 API 契约的唯一定义处 |
| `dream-core-db` | SQLite 数据库层，定义 Repository trait 和实现 |
| `dream-core-assets` | 嵌入式静态资源（Agent 元数据、提示词） |
| `dream-core-runtime` | 托管 Node、子进程管理、PATH 增强 |
| `dream-core-process` | 直连 CLI 会话的子进程监管、隔离和启动清理 |
...
| Crate | 职责 |
|-------|------|
| `dream-core-auth` | JWT 认证、密码哈希、CSRF 保护、Cookie 管理、认证中间件 |
| `dream-core-realtime` | WebSocket 连接管理、事件广播（BroadcastEventBus）、消息路由 |
...
| Crate | 职责 |
|-------|------|
| `dream-core-conversation` | 对话管理、消息收发、确认机制、流式响应 |
| `dream-core-session` | 统一直连 CLI 与 ACP 后端的会话状态、能力、命令和事件 |
| `dream-core-channel` | 多渠道集成（微信、钉钉、飞书）、插件系统、配对会话 |
| `dream-core-team` | 团队协作、任务调度、邮箱系统 |
| `dream-core-team-prompts` | 团队角色提示词、治理规则和工具授权元数据 |
| `dream-core-cron` | 定时任务执行、Cron 表达式、事件触发 |
| `dream-core-file` | 文件操作、监听、快照、Git 操作、压缩 |
| `dream-core-project` | 项目/文件夹绑定、项目浏览器、文件系统监听和资源边界校验 |
| `dream-core-office` | Office 文档处理（Excel、PPT、Word）、预览、转换 |
| `dream-core-system` | 系统设置、提供商管理、版本检查、模型获取 |
| `dream-core-mcp` | MCP 协议集成、OAuth、多平台适配器 |
| `dream-core-ai-agent` | Agent 生命周期管理、Worker 任务队列、ACP/辅助技能 |
...
每个领域 crate 遵循统一的内部组织模式，以 dream-core-conversation 为参考示例：
...
所有响应类型定义在 `dream-core-api-types` 中，这是 API 契约的唯一来源。
...
ACP Agent 的工具调用事件在 `dream-core-ai-agent` 翻译层进入统一
...
所有数据库访问通过 trait 抽象，定义在 `dream-core-db` 中：
...
| 类型 | 位置 | 用途 | 示例 |
|------|------|------|------|
| Row 模型 | `dream-core-db/src/models/` | 数据库行映射 | `ConversationRow` |
| Params 对象 | `dream-core-db/src/repository/` | 数据库写入参数 | `UpdateConversationParams` |
| 请求/响应类型 | `dream-core-api-types` | API 契约与共享 DTO | `CreateConversationRequest`、`ConversationResponse` |

**Service 层可以直接使用 `dream-core-api-types` 中的类型。** 该 crate 是纯数据结构定义，
不依赖任何 HTTP 框架，本质上是共享 DTO 层。

⚠️ **关键约束：`dream-core-api-types` 禁止依赖 axum、tower 等 HTTP 框架，
...
- **Repository（dream-core-db）**：纯数据库操作，不包含业务逻辑
...
- 迁移文件位于 `crates/dream-core-db/migrations/`
...
```
DbError（数据库层）
  ↓ From trait 实现（dream-core-db/src/error.rs）
ApiError（统一错误类型）
  ↓ IntoResponse 实现
...
`dream-core-app` 中定义 `AppServices`，集中持有所有共享依赖：
...
- **领域 crate 不负责构建自己的依赖**——只定义需要什么（RouterState），由 `dream-core-app` 负责组装
...
|--------|----------|--------|----------|---------|
| `dream-session` | ✅ | HTTPS 时 | Strict(HTTPS) / Lax(HTTP) | 30 天 |
| `dream-csrf-token` | ❌ | HTTPS 时 | Strict(HTTPS) / Lax(HTTP) | 30 天 |
...
| E2E 测试 | `crates/dream-core-app/tests/` | 内存 SQLite | 完整 HTTP 请求链路验证 |
...
`dream-core-app/tests/common/mod.rs` 提供共享的测试工具：
...
**第三步：如需数据库，在 dream-core-db 中添加**

1. 在 `models/` 添加 Row 模型
2. 在 `repository/` 定义 Repository trait（`I` 前缀）和 Sqlite 实现
3. 在 `migrations/` 添加迁移文件（`NNN_descriptive_name.sql`）

**第四步：如需 API 类型，在 dream-core-api-types 中添加**

在 `dream-core-api-types` 中定义请求/响应类型，保持 API 契约集中管理。

**第五步：接入 dream-core-app**
...
- [ ] Repository trait 定义在 dream-core-db，实现使用 Sqlite 前缀
- [ ] API 类型定义在 dream-core-api-types
...
内置 ACP adapter 通过 `crates/dream-core-runtime/src/node_runtime/` 中的托管
...
`dream_core_runtime::enhance_process_path()`，使后续所有
...
新的子进程启动点应通过
`dream_core_runtime::Builder::agent(program)`（长期运行的 Agent CLI，
调用者拥有 stdio）或 `dream_core_runtime::Builder::clean_cli(program)`
