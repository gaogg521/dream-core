...
- All request/response types defined in `dream-core-api-types`
- `dream-core-api-types` must NOT depend on axum/tower or any HTTP framework
- Use `dream_core_common::ApiError` only at API/HTTP boundaries such as routes and middleware. Service/domain code must prefer crate-owned errors (`ConversationError`, `TeamError`, etc.) and map them to `ApiError` in route modules.
...
- Repository traits in `dream-core-db`, prefixed with `I`
- Concrete implementations prefixed with `Sqlite`
- Row models in `dream-core-db/src/models/`
...
- All assembly happens in `dream-core-app`'s `build_*_state()` functions
...
New subprocess spawn sites must go through `dream_core_runtime`'s spawn Builder — never raw `tokio::process::Command`. See [ARCHITECTURE.md § Runtime Infrastructure](./ARCHITECTURE.md#runtime-infrastructure) for the correct constructor and details.
...
1. Next number → `ls crates/dream-core-db/migrations/`
2. Create `NNN_descriptive_name.sql` with `IF NOT EXISTS`

### Add WebSocket Event

1. Event type → `dream-core-api-types`
