...
`dream-core-mcp/src/media_workspace.rs` 一直在发 `DREAM_MEDIA_WORKSPACE_DIR` /
`DREAM_MEDIA_CONVERSATION_ID`，而 dream-ui 的 `imageGenServer.ts` 早已改读
...
原先是 6 处硬编码 `temp_dir().join("dream-ui")`，而
...
- `upload_roots()` —— 校验/读取接受的全部目录（`<tmp>/dream` + `<tmp>/dream-ui`）
...
| `one-sessions/` | `dream-engine-sessions/` | `factory/dream_engine.rs` |
| `runtime/one-process/` | `runtime/dream-core-process/` | `registry_store::registry_dir()`、`spawner::local_machine_id()` |
...
## 7. 后续：内置管家/技能改名 + `DREAM_*` env 家族（同日）

### 迁移 053

`dream-core-assistant` → `one-assistant`，四个技能 `dream-ui-{config,troubleshooting,webui-public,webui-setup}`
...
JSON 列（`default_skill_ids` 等）的替换针对带引号 token，否则 `dream-ui-config-extra` 会被误伤。
...
38 个 `DREAM_*` → `ONE_*`。新增 `dream_core_common::legacy_env::adopt_legacy_env()`，
在 `main()` / `admin.rs` 最开头把 `DREAM_X` 采纳为未设的 `ONE_X`——
一处解决 clap `#[arg(env=)]` 和所有 `std::env::var`，读取点全部只认新名。

**安全回归警告**：`registry.rs::is_blocked_override_env_key` 靠 `DREAM_` 前缀阻止用户
env override 覆盖内部变量。改名时必须同步加 `ONE_` 前缀，否则用户能覆盖 `ONE_RUNTIME_TOKEN`。

**批量改名的通用坑**：正则扫描会把「负责兼容旧值」的代码本身也改掉。本次中了两次
（`legacy_env.rs` 的测试、`types.rs` 的 `LEGACY_*` 常量），都改用 `concat!("DREAM", "_X")` 构造。
改完批量改名，务必回头单独检查这类文件。
