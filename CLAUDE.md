...
> 而不是按"有多少他能处理的事"增长，还把真实汇报挤出队长的上下文。
> ❗ 判断"修复装没装上"**别看时间戳，直接在二进制里 grep 那次修复的独有字符串**。
> 已做：源头按槽位去重 + 停止时清掉闲置通知并让队长广播问一圈（只丢 `IdleNotification`，
> 成员汇报是 `Message` 一条不动）。区分表与排查路径见
> [session-2026-09-21-leader-queue-backlog.zh-CN.md](./docs/guides/session-2026-09-21-leader-queue-backlog.zh-CN.md)。
