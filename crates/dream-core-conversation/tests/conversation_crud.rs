...
use dream_core_common::{AgentKillReason, AgentType, ConversationSource, ConversationStatus, TimestampMs};
...
async fn create_acp_seeds_acp_session_runtime_from_extra() {
    use dream_core_db::{SqliteAcpSessionRepository, init_database_memory};
    ...
    let task_mgr: Arc<dyn IWorkerTaskManager> = Arc::new(NoopTaskManager);
    let svc = dream_core_conversation::ConversationService::new(
        std::env::temp_dir(),
        broadcaster.clone(),
        Arc::new(EmptySkillResolver),
        task_mgr,
