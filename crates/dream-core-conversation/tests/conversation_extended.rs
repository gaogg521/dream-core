...
use dream_core_common::{AgentKillReason, ConversationStatus, TimestampMs, generate_prefixed_id, now_ms};
...
use dream_core_db::{IConversationRepository, SqliteConversationRepository, init_database_memory};
...
async fn setup_fork() -> (
    ConversationService,
    Arc<SqliteConversationRepository>,
    Arc<dyn dream_core_db::IAcpSessionRepository>,
) {
    let db = init_database_memory().await.unwrap();
    let repo = Arc::new(SqliteConversationRepository::new(db.pool().clone()));
    let broadcaster = Arc::new(TestBroadcaster::new());
    ...
    for (agent_id, fork_shape) in [
        ...
        agent_metadata_repo
            .apply_handshake(
                agent_id,
                &dream_core_db::UpdateAgentHandshakeParams {
...
fn fork_req(message_id: &str) -> dream_core_api_types::ForkConversationRequest {
...
async fn fork_head_creates_lineage_row_and_copies_messages() {
    ...
    assert_eq!(
        ...
        Some(dream_core_api_types::ForkCapabilityView { at_turn: false }),
...
async fn get_projects_prompt_capability_on_detail_path() {
    ...
    assert_eq!(
        ...
        Some(dream_core_api_types::PromptCapabilityView {
            image: true,
            audio: false
        })
    );
