...
use dream_core_db::{MessagePageCursor, MessagePageDirection, MessagePageParams, MessagePageResult};
use dream_core_extension::{AssistantRuleDispatcher, ExtensionError};
...
impl IConversationRepository for MockRepo {
    async fn get(&self, user_id: &str, id: &str) -> Result<Option<ConversationRow>, dream_core_db::DbError> {
    ...
    async fn create(&self, row: &ConversationRow) -> Result<(), dream_core_db::DbError> {
    ...
    async fn update(
        ...
        let row = rows
            ...
            .ok_or_else(|| dream_core_db::DbError::NotFound(format!("Conversation {id}")))?;
    ...
    async fn delete(&self, user_id: &str, id: &str) -> Result<(), dream_core_db::DbError> {
        let mut rows = self.rows.lock().unwrap();
        let len_before = rows.len();
        rows.retain(|r| !(r.user_id == user_id && r.id == id));
        if rows.len() == len_before {
            return Err(dream_core_db::DbError::NotFound(format!("Conversation {id}")));
        }
        Ok(())
    }
    ...
    ) -> Result<PaginatedResult<ConversationRow>, dream_core_db::DbError> {
        let rows = self.rows.lock().unwrap();
    ...
    async fn find_by_source_and_chat(
        ...
        _chat_id: &str,
        _agent_type: &str,
    ) -> Result<Option<ConversationRow>, dream_core_db::DbError> {
        Ok(None)
    }
    ...
    async fn list_by_cron_job(
        ...
        _user_id: &str,
        _cron_job_id: &str,
    ) -> Result<Vec<ConversationRow>, dream_core_db::DbError> {
        Ok(vec![])
    }
        ...
        let items = match &params.direction {
            ...
            MessagePageDirection::Anchor { message_id } => {
                let anchor = matched
                    ...
                    .cloned()
                    .ok_or_else(|| dream_core_db::DbError::NotFound(format!("Message {message_id}")))?;
    ...
    async fn insert_message(&self, _user_id: &str, message: &MessageRow) -> Result<(), dream_core_db::DbError> {
    ...
    async fn update_message(
        ...
        let message = messages
            ...
            .ok_or_else(|| dream_core_db::DbError::NotFound(format!("Message {id}")))?;
    ...
    async fn get_message_by_msg_id(
        ...
        msg_id: &str,
        msg_type: &str,
    ) -> Result<Option<MessageRow>, dream_core_db::DbError> {
        let messages = self.messages.lock().unwrap();
        Ok(messages
            .iter()
            .find(|message| {
    ...
    async fn list_stale_runtime_messages(
        ...
        Ok(messages
            ...
            .filter_map(|message| {
                ...
                Some(dream_core_db::StaleRuntimeMessageRow {
                    user_id,
    ...
    async fn list_artifacts(
        ...
        _user_id: &str,
        conversation_id: &str,
    ) -> Result<Vec<ConversationArtifactRow>, dream_core_db::DbError> {
        Ok(self
            .artifacts
            .lock()
            .unwrap()
    ...
    async fn get_artifact(
        ...
        conversation_id: &str,
        artifact_id: &str,
    ) -> Result<Option<ConversationArtifactRow>, dream_core_db::DbError> {
        Ok(self
            .artifacts
            .lock()
            .unwrap()
    ...
    async fn list_legacy_cron_trigger_messages(
        ...
        _user_id: &str,
        conversation_id: &str,
    ) -> Result<Vec<MessageRow>, dream_core_db::DbError> {
        Ok(self
            .messages
            .lock()
            .unwrap()
...
async fn make_injected_project_service(
    ...
    .await
    .unwrap();
    let store: std::sync::Arc<dyn dream_core_db::IProjectStore> =
        std::sync::Arc::new(dream_core_db::SqliteProjectStore::new(db.pool().clone()));
    std::mem::forget(db);
    std::sync::Arc::new(dream_core_project::ProjectService::new(
        store,
...
async fn get_reports_idle_runtime_when_only_persisted_status_is_running() {
    ...
    assert_eq!(runtime.state, dream_core_api_types::ConversationRuntimeStateKind::Idle);
...
impl IAgentTask for MockAgent {
    ...
    async fn send_message(&self, _data: SendMessageData) -> Result<(), AgentSendError> {
        ...
        let _ = self.event_tx.send(AgentStreamEvent::Finish(
            dream_core_ai_agent::protocol::events::FinishEventData::default(),
        ));
        Ok(())
    }
...
impl IAgentTask for MidturnMockAgent {
    ...
    fn last_activity_at(&self) -> dream_core_common::TimestampMs {
...
async fn restart_runtime_blocks_duplicate_restart_send_and_config_until_ready() {
    ...
    assert_eq!(
        summary.state,
        dream_core_api_types::ConversationRuntimeStateKind::Restarting
    );
    ...
    assert_eq!(
        ...
        dream_core_api_types::ConversationRuntimeStateKind::Idle
    );
...
async fn send_message_missing_workspace_persists_message_and_failure_tip() {
    ...
    let conv = svc.create("user_1", make_create_req()).await.unwrap();
    broadcaster.take_events();
    let legacy_workspace = format!("/tmp/does-not-exist-{}", dream_core_common::generate_short_id());
    repo.update(
        "user_1",
        &conv.id,
        &ConversationRowUpdate {
...
async fn auto_replay_rebuild_keeps_existing_acp_session_id_in_build_options() {
    ...
    for options in options {
        match options.context.kind {
            ...
            AgentSessionKind::DreamEngine(_) | AgentSessionKind::Antigravity(_) => {
...
async fn cancel_timeout_does_not_kill_non_acp_task() {
    ...
    let conv = svc.create("user_1", make_create_req()).await.unwrap();
    let agent = Arc::new(BlockingCancelAgent::new_with_type(&conv.id, AgentType::DreamEngine));
    task_mgr.insert_agent(&conv.id, AgentInstance::Mock(agent));
...
async fn confirm_removes_confirmation_and_broadcasts() {
    ...
    let req = dream_core_api_types::ConfirmRequest {
        msg_id: "msg-1".into(),
...
async fn create_resolves_assistant_snapshot_and_updates_preferences() {
    ...
    assert_eq!(
        resp.assistant,
        Some(dream_core_api_types::ConversationAssistantIdentityResponse {
            id: "preset-1".into(),
            source: "builtin".into(),
            name: "Preset".into(),
            avatar: "🤖".into(),
...
async fn assistant_backed_aionrs_build_options_include_snapshot_rule_as_preset_rules() {
    ...
    match options.context.kind {
        AgentSessionKind::Acp(_) | AgentSessionKind::Antigravity(_) => {
            panic!("test conversation should build DreamEngine options")
        }
        AgentSessionKind::DreamEngine(ctx) => {
