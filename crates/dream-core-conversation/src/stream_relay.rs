...
use dream_core_ai_agent::protocol::events::{DelegateUsageEventData, ErrorEventData, TipType, TipsEventData};
use dream_core_ai_agent::{AgentSendError, AgentStreamEvent, protocol::events::ThinkingEventData};
...
use dream_core_api_types::{AgentErrorCode, WebSocketMessage};
use dream_core_common::{ErrorChain, normalize_keys_to_snake_case, now_ms};
...
impl StreamRelay {
                ...
                Err(broadcast::error::RecvError::Closed) => {
                    ...
                    let mut outcome = if deleting {
                        ...
                        self.finalize(
                            &full_text_buffer,
                            &text_segments,
                            &AgentStreamEvent::Finish(dream_core_ai_agent::protocol::events::FinishEventData::default()),
    ...
    fn forward_workflow_progress(&self, data: &dream_core_ai_agent::protocol::events::WorkflowProgressData) {
...
mod tests {
    ...
    use dream_core_ai_agent::protocol::events::{
    ...
    fn agent_title_test_row(name: &str, name_source: Option<&str>) -> dream_core_db::models::ConversationRow {
    ...
    async fn needs_auth_tip_sets_summary_flag_on_finish() {
        use dream_core_ai_agent::protocol::events::{TipType, TipsEventData};
    ...
    async fn workflow_progress_does_not_split_the_streaming_reply() {
        use dream_core_ai_agent::protocol::events::tool_call::{ToolGroupEntry, ToolGroupStatus};
        use dream_core_ai_agent::protocol::events::{
    ...
    async fn acp_dialect_signal_is_not_forwarded_or_persisted() {
        use dream_core_ai_agent::protocol::events::{AcpDialectSignalData, AcpDialectSignalKind};
    ...
    async fn run_warning_tip_with_finish_persists_warning_tip() {
        ...
        tx.send(AgentStreamEvent::Tips(
            dream_core_ai_agent::protocol::events::TipsEventData {
                content: String::new(),
                tip_type: dream_core_ai_agent::protocol::events::TipType::Warning,
                code: Some("ACP_EMPTY_TURN".into()),
                params: None,
                supersedes_key: None,
            },
    ...
    async fn run_send_error_injects_error_and_completes_turn() {
        ...
        assert_eq!(
            outcome.terminal,
            RelayTerminal::Error {
                code: Some(dream_core_api_types::AgentErrorCode::UserLlmProviderAuthFailed),
    ...
    async fn run_acp_tool_call_inserts_then_updates() {
        use dream_core_ai_agent::protocol::events::tool_call::{
    ...
    async fn complete_conversation_ignores_not_found_after_delete() {
        ...
        let bus: Arc<dyn EventBroadcaster> = Arc::new(dream_core_realtime::BroadcastEventBus::new(64));
        let adapter =
    ...
    struct RecordingRepo {
        ...
        updates: Mutex<Vec<(String, dream_core_db::MessageRowUpdate)>>,
        /// Optional conversation row served by `get` (agent-title tests).
        conversation: Mutex<Option<dream_core_db::models::ConversationRow>>,
        /// Conversation-level updates recorded by `update` (agent-title tests).
        conversation_updates: Mutex<Vec<(String, dream_core_db::ConversationRowUpdate)>>,
    ...
    impl RecordingRepo {
        ...
        #[allow(dead_code)]
        fn take_updates(&self) -> Vec<(String, dream_core_db::MessageRowUpdate)> {
    ...
    impl IConversationRepository for RecordingRepo {
        ...
        async fn create(&self, _row: &dream_core_db::models::ConversationRow) -> Result<(), DbError> {
            Ok(())
        }
        async fn update(
            &self,
            _user_id: &str,
            id: &str,
            updates: &dream_core_db::ConversationRowUpdate,
        ) -> Result<(), DbError> {
        ...
        async fn list_paginated(
            &self,
            _user_id: &str,
            _filters: &dream_core_db::ConversationFilters,
        ) -> Result<dream_core_common::PaginatedResult<dream_core_db::models::ConversationRow>, DbError> {
            Ok(dream_core_common::PaginatedResult {
                items: vec![],
                total: 0,
                has_more: false,
            })
        ...
        async fn list_by_cron_job(
            ...
            _user_id: &str,
            _cron_job_id: &str,
        ) -> Result<Vec<dream_core_db::models::ConversationRow>, DbError> {
            Ok(vec![])
        }
        ...
        async fn list_messages_page(
            ...
            _user_id: &str,
            _conv_id: &str,
            _params: &dream_core_db::MessagePageParams,
        ) -> Result<dream_core_db::MessagePageResult, DbError> {
            Ok(dream_core_db::MessagePageResult {
                items: vec![],
        ...
        async fn upsert_message(&self, _user_id: &str, row: &MessageRow) -> Result<(), DbError> {
            ...
            if let Some(existing) = inserts.iter().find(|message| message.id == row.id) {
                ...
                self.updates.lock().unwrap().push((
                    row.id.clone(),
                    dream_core_db::MessageRowUpdate {
        ...
        async fn update_message(
            ...
            _conversation_id: &str,
            id: &str,
            updates: &dream_core_db::MessageRowUpdate,
        ) -> Result<(), DbError> {
            if self.not_found.load(Ordering::Acquire) {
        ...
        async fn search_messages(
            ...
            _page: u32,
            _page_size: u32,
        ) -> Result<dream_core_common::PaginatedResult<dream_core_db::MessageSearchRow>, DbError> {
            Ok(dream_core_common::PaginatedResult {
                items: vec![],
                total: 0,
                has_more: false,
