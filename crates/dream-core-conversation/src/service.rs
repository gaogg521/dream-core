...
use dream_core_ai_agent::session_context::{AgentSessionContext, AgentSessionKind};
...
use dream_core_extension::AssistantRuleDispatcher;
...
fn assistant_snapshot_modes<'a>(
    ...
    definition: &'a dream_core_db::AssistantDefinitionRow,
...
impl ConversationService {
    ...
    async fn static_supports_midturn_delivery(&self, conversation_id: &str) -> bool {
        ...
        serde_json::from_str::<serde_json::Value>(&row.extra)
            ...
            .is_some_and(dream_core_ai_agent::backend_supports_midturn_delivery)
    }
...
impl ConversationService {
    ...
    async fn assistant_identity_from_snapshot(
    ...
    ) -> Result<dream_core_api_types::ConversationAssistantIdentityResponse, ConversationError> {
        ...
        Ok(dream_core_api_types::ConversationAssistantIdentityResponse {
    ...
    pub async fn create(
        ...
        let row = dream_core_db::models::ConversationRow {
            id: id.clone(),
            user_id: user_id.to_owned(),
        ...
        if let Some(snapshot) = assistant_snapshot.as_ref() {
            response.assistant = Some(dream_core_api_types::ConversationAssistantIdentityResponse {
                id: snapshot.assistant_id.clone(),
    ...
    async fn persist_assistant_preferences_from_snapshot(
        ...
        preference_repo
            .upsert_for_user(
                user_id,
                &dream_core_db::UpsertAssistantPreferenceParams {
                    assistant_definition_id: &snapshot.assistant_definition_id,
    ...
    pub async fn update(
        ...
        let merged_extra = if let Some(new_extra) = &req.extra {
            ...
            if existing_type == AgentType::DreamEngine
    ...
    pub async fn fork(
        ...
        if let Some(obj) = extra.as_object_mut() {
            obj.insert(
                "fork".to_owned(),
                serde_json::to_value(dream_core_api_types::ForkSpec {
    ...
    async fn agent_capabilities_for_agent(
        ...
        let backend = dream_core_db::runtime_backend_for_agent(&metadata_row);
        let persisted = metadata_row
            .agent_capabilities
            .as_deref()
            .and_then(|capabilities| serde_json::from_str::<serde_json::Value>(capabilities).ok());
        Ok(dream_core_ai_agent::effective_agent_capabilities(
            &backend,
            persisted.as_ref(),
        ))
    }
...
pub(crate) async fn apply_message_receipt(
    ...
    let payload = dream_core_api_types::MessageStatusChangedPayload {
        user_id: user_id.to_owned(),
        conversation_id: conversation_id.to_owned(),
...
impl ConversationService {
    ...
    async fn deliver_midturn_message(
        ...
        if persisted {
            let user_msg = dream_core_db::models::MessageRow {
...
impl ConversationService {
    ...
    pub async fn resolve_assistant_mcp_selection(
        ...
        for id in &selected_ids {
            ...
            if row.builtin {
                match dream_core_ai_agent::mcp_resolve::row_to_session_mcp_server(&row).await {
...
async fn resolve_acp_mcp_support_policy(
    ...
    let effective_backend = row
        .as_ref()
        .map(dream_core_db::runtime_backend_for_agent)
        .or_else(|| backend.map(str::to_owned))
        .unwrap_or_default();
    let capabilities = dream_core_ai_agent::effective_agent_capabilities(&effective_backend, persisted.as_ref())
        .as_ref()
...
mod tests {
    ...
    fn response_with_type(agent_type: dream_core_common::AgentType) -> ConversationResponse {
        ...
        let status = classify_session_mcp_status(
            &SessionMcpServer {
                ...
                transport: SessionMcpTransport::Stdio {
                    command: "__definitely_missing_dream_core_mcp_command__".into(),
                    args: Vec::new(),
                    env: HashMap::new(),
                },
            },
