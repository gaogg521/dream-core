...
/// Production adapter backed by `dream_core_extension::skill_service`.
...
impl ExtensionSkillResolver {
    pub fn new(paths: Arc<dream_core_extension::SkillPaths>, skill_repo: Arc<dyn ISkillRepository>) -> Self {
...
impl SkillResolver for ExtensionSkillResolver {
    async fn auto_inject_names(&self) -> Vec<String> {
        match dream_core_extension::list_available_skills_with_repo(&self.paths, self.skill_repo.as_ref()).await {
            Ok(items) => {
    ...
    async fn resolve_skills_for_user(&self, user_id: &str, names: &[String]) -> Vec<ResolvedAgentSkill> {
        ...
        match dream_core_extension::materialize_skills_for_agent_with_repo_for_user(
            &self.paths,
    ...
    async fn link_workspace_skills(&self, workspace: &Path, rel_dirs: &[&str], skills: &[ResolvedAgentSkill]) -> usize {
        ...
        match dream_core_extension::link_workspace_skills(workspace, rel_dirs, skills).await {
            Ok(n) => n,
            Err(e) => {
                tracing::warn!(
...
mod tests {
    use super::*;
    use dream_core_db::{SqliteSkillRepository, UpsertSkillParams};
