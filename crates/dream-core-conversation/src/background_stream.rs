...
use dream_core_ai_agent::protocol::events::MessageLifecyclePhase;
use dream_core_ai_agent::protocol::events::{AgentStreamEvent, WorkflowProgressData};
use dream_core_common::{ErrorChain, normalize_keys_to_snake_case};
...
impl BackgroundStreamWatcher {
    ...
    fn forward_card_frames(&self, data: &WorkflowProgressData) {
        for (kind, msg_id, body) in [
            ...
            self.broadcaster.broadcast(dream_core_api_types::WebSocketMessage::new(
                "message.stream",
                json!({
                    "conversation_id": self.conversation_id,
...
mod tests {
    ...
    use dream_core_ai_agent::protocol::events::{FinishEventData, MessageLifecycleData, TextEventData};
    ...
    struct Rig {
        ...
        bus: Arc<dream_core_realtime::BroadcastEventBus>,
    ...
    async fn rows_of_type(rig: &Rig, ty: &str) -> Vec<dream_core_db::models::MessageRow> {
        rig.repo
            .list_messages_page(
                &rig.user_id,
                "conv-1",
    ...
    async fn between_turns_config_option_snapshot_is_forwarded() {
        ...
        let deadline = std::time::Instant::now() + std::time::Duration::from_secs(3);
        let mut forwarded: Option<dream_core_api_types::WebSocketMessage<serde_json::Value>> = None;
