use dream_core_common::constants::{COOKIE_MAX_AGE_DAYS, COOKIE_NAME, CSRF_COOKIE_NAME};
...
impl CookieConfig {
    ...
    ///
    /// - `ONE_HTTPS=true` → Secure flag, SameSite=Strict
    /// - Otherwise → no Secure flag, SameSite=Lax (for remote HTTP access)
    pub fn from_env() -> Self {
        let https = std::env::var("ONE_HTTPS")
...
mod tests {
    ...
    fn session_cookie_http() {
        ...
        assert!(cookie.contains("dream-session=my_token"));
    ...
    fn clear_session_cookie_sets_max_age_zero() {
        ...
        assert!(cookie.contains("dream-session="));
    ...
    fn csrf_cookie_not_http_only() {
        ...
        assert!(cookie.contains("dream-csrf-token=csrf_abc"));
        assert!(!cookie.contains("HttpOnly"));
        assert!(cookie.contains("SameSite=Lax"));
        assert!(cookie.contains("Max-Age="));
    }
