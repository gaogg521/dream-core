...
use dream_core_db::{IUserRepository, SqliteUserRepository, UserStatus, init_database_memory};
...
fn extract_session_token(resp: &axum::response::Response) -> Option<String> {
    resp.headers()
        ...
        .find_map(|cookie| {
            cookie
                .split(',')
                .find(|part| part.trim_start().starts_with("dream-session="))
                .and_then(|part| part.trim_start().strip_prefix("dream-session="))
...
async fn t4_1_login_success() {
    ...
    assert!(set_cookie.contains("dream-session="));
