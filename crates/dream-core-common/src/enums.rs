...
pub enum AgentType {
    ...
    /// vendor, and unlike `DreamEngine` it does spawn an external process.
    Antigravity,
...
impl AgentType {
    ...
    pub fn supports_new_conversation(&self) -> bool {
        matches!(self, AgentType::Acp | AgentType::DreamEngine | AgentType::Antigravity)
    }
...
mod tests {
    ...
    fn test_agent_type_display_names() {
        ...
        assert_eq!(AgentType::DreamEngine.display_name(), "1ONE CLI");
    ...
    fn test_agent_type_id_stability() {
        let id = AgentType::DreamEngine.id();
        assert_eq!(id.len(), 8);
        assert!(id.chars().all(|c| c.is_ascii_hexdigit()));
        assert_eq!(AgentType::DreamEngine.id(), AgentType::DreamEngine.id());
    }
    ...
    fn agent_type_new_conversation_support_policy_is_explicit() {
        ...
        assert!(AgentType::DreamEngine.supports_new_conversation());
    ...
    fn agent_type_deprecated_runtime_policy_matches_new_conversation_support() {
        ...
        assert!(!AgentType::DreamEngine.is_deprecated_runtime());
    ...
    fn agent_type_full_auto_mode_id_supports_non_acp_agents() {
        ...
        assert_eq!(AgentType::DreamEngine.full_auto_mode_id(None), "yolo");
