...
use dream_core_api_types::{McpServerResponse, McpToolResponse, McpTransport};
...
impl McpTool {
    ...
    pub fn is_model_api_compatible(&self) -> bool {
        dream_core_common::is_tool_compatible(&self.name, self.input_schema.as_ref())
    }
}
...
pub fn incompatible_tools_in_persisted_json(tools_json: &str) -> Vec<IncompatibleTool> {
    ...
    tools
        .into_iter()
        .flat_map(|tool| {
            dream_core_common::validate_tool(&tool.name, tool.input_schema.as_ref())
                .into_iter()
