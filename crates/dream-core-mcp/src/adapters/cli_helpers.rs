...
use dream_core_runtime::resolve_command_path;
...
///
/// Uses `dream_core_runtime::resolve_command_path` so the lookup respects
...
/// [`dream_core_common::agent_bridge`].
...
mod tests {
    ...
    async fn is_cli_installed_returns_false_for_missing_binary() {
        let result = is_cli_installed("one-definitely-not-a-real-binary-xyz").await.unwrap();
        assert!(!result);
    }
