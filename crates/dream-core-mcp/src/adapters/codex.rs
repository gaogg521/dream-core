...
/// | `install_server` / `remove_server` | isolated home (`dream_core_common::codex_mcp_isolated_home`) | **Never** the real home. |
///
...
impl CodexAdapter {
    ...
    pub fn new(data_dir: &Path) -> Self {
        Self {
            config_dir: dream_core_common::codex_mcp_isolated_home(data_dir),
        }
    }
    ...
    fn isolated_env(&self) -> [(&'static str, String); 1] {
        [(
            dream_core_common::CODEX_HOME_ENV_KEY,
...
mod tests {
    ...
    fn adapter_targets_the_isolated_codex_mcp_home_not_an_ad_hoc_path() {
        ...
        assert_eq!(adapter.config_dir, dream_core_common::codex_mcp_isolated_home(data_dir));
    }
    ...
    fn mutations_are_pinned_to_the_isolated_home() {
        ...
        assert_eq!(env.len(), 1);
        assert_eq!(env[0].0, dream_core_common::CODEX_HOME_ENV_KEY);
        assert!(
            env[0].1.contains(dream_core_common::CODEX_MCP_ISOLATED_HOME_DIR_NAME),
    ...
    fn isolated_home_is_distinct_from_claude_bridge_home() {
        ...
        assert_ne!(
            ...
            dream_core_common::claude_bridge_home(data_dir)
        );
    }
