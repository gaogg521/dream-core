...
use dream_core_common::McpServerStatus;
use dream_core_mcp::{
...
fn si_7_builtin_image_gen_injection() {
    ...
    match builtin {
        AcpSessionMcpServer::Stdio { name, command, env, .. } => {
            ...
            assert_eq!(env_map["ONE_IMG_MODEL"], "dall-e-3");
            assert_eq!(env_map["ONE_IMG_API_URL"], "https://api.openai.com/v1");
            assert_eq!(env_map["ONE_IMG_API_KEY"], "sk-test-key");
            assert_eq!(env_map["ONE_IMG_SIZE"], "1024x1024");
            assert_eq!(env_map["ONE_IMG_QUALITY"], "hd");
            assert_eq!(env_map["ONE_IMG_STYLE"], "natural");
        }
