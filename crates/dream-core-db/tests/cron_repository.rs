...
async fn insert_user_and_conversation(db: &dream_core_db::Database, user_id: &str, conversation_id: &str) {
    sqlx::query(
        "INSERT INTO users (id, username, password_hash, created_at, updated_at) \
         VALUES (?, ?, 'hash', 0, 0)",
    )
