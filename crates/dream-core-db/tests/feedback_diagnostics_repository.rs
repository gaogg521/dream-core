...
async fn insert_feedback_fixture(db: &dream_core_db::Database) {
    let pool = db.pool();
    sqlx::query(
...
async fn insert_mcp_feedback_fixture(db: &dream_core_db::Database) {
...
async fn insert_foreign_agent_metadata(db: &dream_core_db::Database) {
    sqlx::query(
        "INSERT INTO users \
