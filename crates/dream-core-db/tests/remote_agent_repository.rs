...
async fn repo() -> (Arc<dyn IRemoteAgentRepository>, dream_core_db::Database) {
...
async fn update_status_to_connected_with_timestamp() {
    ...
    let ts = dream_core_common::now_ms();
...
async fn full_crud_lifecycle() {
    ...
    // Update status
    r.update_status(USER_ID, &created.id, "connected", Some(dream_core_common::now_ms()))
        .await
        .unwrap();
