...
async fn setup() -> (SqliteConversationRepository, dream_core_db::Database) {
    let db = init_database_memory().await.unwrap();
    let repo = SqliteConversationRepository::new(db.pool().clone());
    (repo, db)
}

async fn create_user_2(db: &dream_core_db::Database) {
    sqlx::query(
...
fn make_conversation(suffix: &str) -> ConversationRow {
    ...
    ConversationRow {
        id: dream_core_common::generate_prefixed_id("conv"),
...
fn make_message(conv_id: &str, content: &str) -> MessageRow {
    let now = dream_core_common::now_ms();
    MessageRow {
        id: dream_core_common::generate_prefixed_id("msg"),
        conversation_id: conv_id.to_string(),
        msg_id: Some(dream_core_common::generate_prefixed_id("cmsg")),
...
fn make_artifact(conv_id: &str, artifact_id: &str) -> dream_core_db::ConversationArtifactRow {
...
async fn pin_and_unpin_conversation() {
    ...
    // Pin
    let pin_time = dream_core_common::now_ms();
    repo.update(
        &conv.user_id,
        &conv.id,
        &ConversationRowUpdate {
