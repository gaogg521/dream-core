...
pub struct AgentMetadataRow {
    ...
    /// (`dream-common/src/enums.rs`), a hardcoded per-backend table whose
    /// default arm is the literal `"yolo"` — see Team spawn
    /// (`dream-team/src/service/spawn_support.rs`) and cron
    /// (`dream-cron/src/service.rs`). So clearing this column silently hands
