...
use dream_core_ai_agent::{ActiveLeaseRegistry, AgentError, IWorkerTaskManager, WorkerTaskManagerImpl};
...
use dream_core_team::session::SpawnAgentRequest;
use dream_core_team::{
...
impl TeamConversationProvisioningPort for FakeConversationPorts {
    ...
    ) -> Result<TeamConversationCreateResult, dream_core_team::TeamError> {
        if request.assistant_id.is_some() && request.agent_type.is_some() {
            return Err(dream_core_team::TeamError::InvalidRequest(
                "assistant-backed team conversations must not provide agent_type".into(),
            ));
        }
        let id = dream_core_common::generate_id();
    ...
    async fn resolve_assistant_mcp_selection(
        ...
        _user_id: &str,
        assistant_id: &str,
    ) -> Result<Option<TeamMcpSelection>, dream_core_team::TeamError> {
        Ok(Some(
            self.assistant_mcp_selections
                .lock()
                .unwrap()
    ...
    ) -> Result<TeamMcpSnapshotResolution, dream_core_team::TeamError> {
    ...
    async fn create_team_temp_workspace(
        ...
        _user_id: &str,
        team_id: &str,
    ) -> Result<String, dream_core_team::TeamError> {
        if self
    ...
    async fn patch_runtime_config(
    ...
    ) -> Result<(), dream_core_team::TeamError> {
        ...
        let user_id = self
            ...
            .await?
            .ok_or_else(|| dream_core_team::TeamError::AgentNotFound(conversation_id.to_owned()))?;
        self.repo
            .update(
                &user_id,
                conversation_id,
                &ConversationRowUpdate {
                    ...
                    status: None,
                    updated_at: Some(dream_core_common::now_ms()),
                    project_id: None,
                    folder_id: None,
                    name_source: None,
                },
    ...
    async fn conversation_model_facts(
        &self,
        conversation_id: &str,
    ) -> Result<dream_core_team::TeamConversationModelFacts, dream_core_team::TeamError> {
        let extra = self
            .repo
            .get_extra(conversation_id)
            .ok_or_else(|| dream_core_team::TeamError::AgentNotFound(conversation_id.to_owned()))?;
        ...
        Ok(dream_core_team::TeamConversationModelFacts {
            confirmed_model_id: value("confirmed_model_id").or_else(|| value("current_model_id")),
            runtime_seed_model_id: value("current_model_id"),
        })
    }

    async fn save_acp_runtime_mode(&self, conversation_id: &str, mode: &str) -> Result<(), dream_core_team::TeamError> {
    ...
    async fn get_config_options(
        &self,
        conversation_id: &str,
    ) -> Result<GetConfigOptionsResponse, dream_core_team::TeamError> {
        let extra = self
            .repo
            .get_extra(conversation_id)
            .ok_or_else(|| dream_core_team::TeamError::AgentNotFound(conversation_id.to_owned()))?;
    ...
    ) -> Result<SetConfigOptionResponse, dream_core_team::TeamError> {
    ...
    async fn warmup_agent_process(
        ...
        let row = self.repo.get(user_id, conversation_id).await?.ok_or_else(|| {
            dream_core_team::TeamError::InvalidRequest(format!("conversation not found: {conversation_id}"))
        })?;
        let extra: serde_json::Value = serde_json::from_str(&row.extra)?;
        let team = dream_core_api_types::TeamSessionBinding::from_extra_value(&extra)?;
        ...
        task_manager
            ...
            .await
            .map_err(|error| {
                dream_core_team::TeamError::InvalidRequest(format!("failed to warm up agent process: {error}"))
            })?;
        Ok(())
    }
...
impl TeamProjectionMessageStore for FakeConversationPorts {
    ...
    async fn find_projected_message(
        ...
        msg_id: &str,
        msg_type: &str,
    ) -> Result<Option<MessageRow>, dream_core_team::TeamError> {
    ...
    async fn insert_projected_message(&self, row: &MessageRow) -> Result<(), dream_core_team::TeamError> {
        let user_id = self
            .repo
            .owner_user_id(&row.conversation_id)
            .await?
            .ok_or_else(|| dream_core_team::TeamError::InvalidRequest("conversation not found".into()))?;
...
impl TeamConversationLookupPort for FakeConversationPorts {
    async fn lookup_team_binding_by_conversation(
        &self,
        conversation_id: &str,
    ) -> Result<Option<TeamConversationBindingLookup>, dream_core_team::TeamError> {
        let Some(user_id) = self.repo.owner_user_id(conversation_id).await? else {
            return Ok(None);
        };
...
struct FullMockTeamRepo {
    inner: MockTeamRepo,
    teams: std::sync::Mutex<Vec<dream_core_db::models::TeamRow>>,
...
impl ITeamRepository for FullMockTeamRepo {
    async fn create_team(&self, row: &dream_core_db::models::TeamRow) -> Result<(), DbError> {
        self.teams.lock().unwrap().push(row.clone());
        Ok(())
    }
    async fn list_teams_for_restore(&self) -> Result<Vec<dream_core_db::models::TeamRow>, DbError> {
        Ok(self.teams.lock().unwrap().clone())
    }
    async fn list_teams_by_user(&self, user_id: &str) -> Result<Vec<dream_core_db::models::TeamRow>, DbError> {
        Ok(self
            .teams
            .lock()
            .unwrap()
    ...
    async fn get_team(&self, user_id: &str, id: &str) -> Result<Option<dream_core_db::models::TeamRow>, DbError> {
        Ok(self
            .teams
            .lock()
            .unwrap()
    ...
    async fn update_team(
        ...
        team.updated_at = dream_core_common::now_ms();
        Ok(())
    }
    ...
    async fn create_task(&self, user_id: &str, row: &dream_core_db::models::TeamTaskRow) -> Result<(), DbError> {
    ...
    async fn find_task_by_id(
        ...
        team_id: &str,
        task_id: &str,
    ) -> Result<Option<dream_core_db::models::TeamTaskRow>, DbError> {
    ...
    async fn update_task(
        ...
        team_id: &str,
        task_id: &str,
        params: &dream_core_db::UpdateTaskParams,
    ) -> Result<(), DbError> {
...
impl IWorkerTaskManager for CountingTaskManager {
    fn get_task(&self, conversation_id: &str) -> Option<dream_core_ai_agent::AgentInstance> {
    ...
    fn collect_idle(&self, idle_threshold_ms: dream_core_common::TimestampMs) -> Vec<String> {
...
mod mock_agent {
    ...
    use dream_core_ai_agent::agent_task::{IAgentTask, IMockAgent};
    use dream_core_ai_agent::protocol::events::AgentStreamEvent;
    use dream_core_ai_agent::types::SendMessageData;
    use dream_core_common::{AgentKillReason, AgentType, Confirmation, ConversationStatus, TimestampMs};
    ...
    impl IAgentTask for MockAgent {
        ...
        async fn send_message(&self, _data: SendMessageData) -> Result<(), dream_core_ai_agent::AgentSendError> {
            Ok(())
        }
...
fn success_factory() -> AgentFactory {
    ...
    Arc::new(|opts: BuildTaskOptions| {
        async move {
            Ok(dream_core_ai_agent::AgentInstance::Mock(Arc::new(
...
impl IProviderRepository for EmptyProviderRepo {
    ...
    async fn create(
        &self,
        _params: dream_core_db::CreateProviderParams<'_>,
    ) -> Result<dream_core_db::models::Provider, DbError> {
    ...
    async fn update(
        ...
        _user_id: &str,
        _id: &str,
        _params: dream_core_db::UpdateProviderParams<'_>,
    ) -> Result<dream_core_db::models::Provider, DbError> {
        Err(DbError::NotFound("not implemented".into()))
    }
...
async fn recovery_creates_system_run_intents_without_restoring_old_memory_run() {
    ...
    team_repo
        .write_message(
            "user1",
            &dream_core_db::models::MailboxMessageRow {
                ...
                files: None,
                read: false,
                created_at: dream_core_common::now_ms(),
            },
        )
        .await
...
async fn renew_active_lease_allows_empty_team_without_unrelated_lease() {
    ...
    team_repo
        .create_team(&dream_core_db::models::TeamRow {
            id: "team-empty".into(),
            user_id: "user1".into(),
            name: "Empty".into(),
            ...
            created_at: dream_core_common::now_ms(),
            updated_at: dream_core_common::now_ms(),
            project_id: None,
            folder_id: None,
        })
        .await
...
async fn force_team_workspace(repo: &Arc<FullMockTeamRepo>, team_id: &str, workspace: &str) {
    repo.update_team(
        "user1",
        team_id,
        &dream_core_db::UpdateTeamParams {
...
async fn d31b2_ensure_session_broadcasts_failed_loading_team_for_missing_team() {
    ...
    assert!(matches!(err, dream_core_team::TeamError::TeamNotFound(_)));
...
fn activity_team_row(id: &str, user_id: &str) -> dream_core_db::models::TeamRow {
    dream_core_db::models::TeamRow {
        id: id.into(),
...
fn activity_message_row(id: &str, team_id: &str, created_at: i64) -> dream_core_db::models::MailboxMessageRow {
    dream_core_db::models::MailboxMessageRow {
        id: id.into(),
...
fn activity_task_row(id: &str, team_id: &str, created_at: i64) -> dream_core_db::models::TeamTaskRow {
    dream_core_db::models::TeamTaskRow {
        id: id.into(),
        team_id: team_id.into(),
...
async fn activity_all_merges_and_orders_desc_with_cursor() {
    ...
    let page = svc
        .list_team_activity(
            ...
            "t1",
            None,
            dream_core_db::PageDirection::Desc,
            dream_core_team::ActivityKind::All,
            3,
        )
        .await
        .unwrap();
    ...
    let page2 = svc
        .list_team_activity(
            "user1",
            "t1",
            Some(dream_core_db::ActivityCursor {
                created_at: c.ts,
                id: c.id,
            }),
            dream_core_db::PageDirection::Desc,
...
async fn activity_kind_task_only_paginates_tasks() {
    ...
    let page = svc
        .list_team_activity(
            ...
            None,
            dream_core_db::PageDirection::Desc,
            dream_core_team::ActivityKind::Task,
            10,
        )
        .await
        .unwrap();
