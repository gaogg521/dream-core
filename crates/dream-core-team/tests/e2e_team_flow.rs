...
use dream_core_db::ITeamRepository;
use dream_core_realtime::EventBroadcaster;
use dream_core_team::event_loop::AgentLoopContext;
...
impl TeamProjectionMessageStore for NoopProjectionStore {
    ...
    async fn find_projected_message(
        ...
        _msg_id: &str,
        _msg_type: &str,
    ) -> Result<Option<dream_core_db::models::MessageRow>, dream_core_team::TeamError> {
        Ok(None)
    }

    async fn insert_projected_message(
        &self,
        _row: &dream_core_db::models::MessageRow,
...
#[async_trait]
impl dream_core_ai_agent::IWorkerTaskManager for StubTaskManager {
...
fn backend_path() -> Arc<PathBuf> {
    Arc::new(PathBuf::from("/tmp/dreamcore-e2e-test"))
}
...
async fn setup_session_with_turn_recorder_inner(
    ...
    let task_manager_dyn: Arc<dyn dream_core_ai_agent::IWorkerTaskManager> = task_manager.clone();
    ...
    let team = dream_core_team::types::Team {
        id: "e2e-team".into(),
        name: "E2E Team".into(),
...
async fn setup_session_with_runtime_ports(
    ...
    let task_manager_dyn: Arc<dyn dream_core_ai_agent::IWorkerTaskManager> = task_manager;
    let team = dream_core_team::types::Team {
        id: "e2e-team".into(),
        name: "E2E Team".into(),
        workspace: "/tmp/e2e-team".into(),
...
async fn s3a_on_agent_finish_writes_idle_notification_to_lead() {
    ...
    session
        .scheduler()
        .set_status("worker-1", dream_core_team::TeammateStatus::Working)
        .await
        .unwrap();
...
async fn s3b_lead_finish_does_not_write_idle_notification() {
    ...
    session
        .scheduler()
        .set_status("lead-1", dream_core_team::TeammateStatus::Working)
        .await
        .unwrap();
...
async fn s3c_finish_triggers_lead_wake_with_idle_notification() {
    ...
    session
        ...
        .write(
            ...
            "worker-1",
            "lead-1",
            dream_core_team::MailboxMessageType::Message,
            "do the work",
            None,
        )
        .await
...
async fn repeated_start_failure_stops_after_three_attempts_and_marks_slot_error() {
    ...
    assert_eq!(
        ...
        dream_core_team::TeammateStatus::Error
    );
