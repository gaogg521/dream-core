...
use dream_core_db::{ITeamRepository, SqliteTeamRepository, init_database_memory};
use dream_core_team::{TaskBoard, TaskStatus, TaskUpdate};

async fn setup() -> (TaskBoard, dream_core_db::Database) {
    let db = init_database_memory().await.unwrap();
    let repo = Arc::new(SqliteTeamRepository::new(db.pool().clone()));
    repo.create_team(&dream_core_db::models::TeamRow {
        id: "t1".to_owned(),
