...
**双保险：**
1. `crates/dream-core-team/src/guide/server.rs:156` — send_message 追加 `[SYSTEM NOTE]` 明确指示直接 spawn
2. `crates/dream-core-team/src/prompts/prompt_templates/lead.txt` — 规则 9/10/88 增加 SYSTEM NOTE 例外
...
- `crates/dream-core-team/src/service.rs` — create_team 成功后 broadcast `team.created` 事件
...
- `crates/dream-core-team/src/service.rs` — `rebuild_agent_processes` 的 patch 加 `session_mode`
- `crates/dream-core-team/src/session.rs` — `attach_spawned_agent_process_bg` 的 patch 加 `session_mode`
...
`crates/dream-core-team/src/mcp/server.rs` — `exec_shutdown_agent` 写入 mailbox 后调用 `svc.wake_agent_in_session(team_id, &target_slot_id)` 唤醒目标。
...
`crates/dream-core-team/src/service.rs` — 新增 `ensure_session_locks: DashMap<String, Mutex>` 字段，`ensure_session` 加 per-team mutex + double-check locking。
...
`crates/dream-core-app/src/guide_stdio.rs` — `forward_tool` 加最多 3 次重试（间隔 500ms/1s/1.5s）。
...
```
crates/dream-core-team/src/service.rs          — wake_lock 修复、StreamRelay、ensure_session 锁、team.created 事件、conversation.listChanged、session_mode
crates/dream-core-team/src/session.rs          — try_wake 加 StreamRelay、移除 leader mirror 跳过、session_mode、msg_id 去重
crates/dream-core-team/src/guide/server.rs     — send_message 加 [SYSTEM NOTE]
crates/dream-core-team/src/prompts/prompt_templates/lead.txt — SYSTEM NOTE 例外规则
crates/dream-core-team/src/mcp/server.rs       — shutdown_agent 加 wake
crates/dream-core-team/docs/phase2/bugfix.md   — 本文档
crates/dream-core-conversation/src/service.rs  — pub fn repo() accessor
crates/dream-core-app/src/guide_stdio.rs       — forward_tool 重试逻辑
```
