...
2. D3 在 `crates/dream-core-team/src/mcp/bridge.rs` PR 里附一段注释：`// SDK McpServer variant shape (as of agent-client-protocol-schema 0.12.0): ...`
...
**产出物清单**（10 个模块）：
- ⬜ D1 `dream-core-api-types::team_mcp` 合并 + 单元测试绿
...
**产出物清单**：
- ⬜ D11 smoke test `crates/dream-core-app/tests/team_phase1_smoke.rs` 合并
...
4. `conversation.send_message` 发给 team 成员 conversation → 能看到 `RUST_LOG=dream_core_conversation=debug` 里 `routing to team path` 的日志

**关键对齐点**：
- W3-D16 的 trait `ITeamMessageRouter` 放 `dream-core-conversation` 或中间层，**禁止** `dream-core-conversation` 反向依赖 `dream-core-team`
- W3-D14 的 `renamed_agents` map 要能被 W2 D5 的 prompt builder 读到；若 D5 在 W2 阶段未预留参数，M5 里回头补 builder 参数（按 [interface-contracts.md §5](./interface-contracts.md#5-dream-core-teamprompts-大幅扩写wave-1--模块-d5) 本来就有）

**不通过的典型信号**：
- W3-D12 的实现把越权访问暴露成 `Forbidden` 而不是 `NotFound` → 驳回（信息泄漏）
- W3-D16 的 trait 误放 `dream-core-team` 导致 `dream-core-conversation → dream-core-team` 循环依赖 → 驳回
...
- ⬜ **可靠性 smoke test** `crates/dream-core-app/tests/team_reliability_smoke.rs` 通过
...
2. `RUST_LOG=dream_core_team=debug` 运行日志显示所有 8 个 phase 的触发点
...
- ⬜ **全量 e2e smoke test** `crates/dream-core-app/tests/team_full_e2e_smoke.rs` 通过
