...
| 类型 | 目标 |
|------|------|
| 新增 | `dream-core-api-types::team_mcp` 子模块（提升 `TeamMcpStdioConfig`） |
| 新增 | `dream-core-team::mcp::bridge` 新 struct `TeamMcpStdioServerSpec`（session/new 注入体） |
| 新增 | `dream-core-team::prompts` 4 份常量字符串（leader/teammate/guide/spawn tool desc）+ builder 新签名 |
| 新增 | `dream-core-team::mcp::tools` 两个工具 descriptor + handler：`team_list_models` / `team_describe_assistant` |
| 新增 | `dream-core-team::session::TeamSession::stdio_spec(slot_id)` 返回 `TeamMcpStdioServerSpec` |
| 新增 | `dream-core-team::session::TeamSession::on_agent_finish(conversation_id, is_error)` 供 Wave 2 转发 Finish |
| 新增 | `dream-core-app` 子命令 `mcp-bridge`（stdio bridge 入口，无新 trait） |
| 修改 | `dream-core-ai-agent::AcpBuildExtra` 加字段 `team_mcp_stdio_config: Option<TeamMcpStdioConfig>` |
| 修改 | `dream-core-ai-agent::acp_agent::session_new_and_prompt` 按 config 注入 mcp_servers |
| 修改 | `dream-core-team::service::TeamSessionService::new` 加 `task_manager: Arc<dyn IWorkerTaskManager>` 入参 |
| 修改 | `dream-core-team::service::ensure_session` 实现 kill+rebuild 闭环（写回 extra → kill → get_or_build_task） |
| 修改 | `dream-core-team::session::TeamSession::send_message / send_message_to_agent` 接 wake 路径 |
...
## 1. `dream-core-api-types` 新增类型（Wave 1 · 模块 D1）

**文件**：新增 `crates/dream-core-api-types/src/team_mcp.rs`；在 `lib.rs` `pub mod team_mcp; pub use team_mcp::*;`

```rust
use serde::{Deserialize, Serialize};

/// team session MCP server 的 stdio 连接三元组
/// (提升自 dream-core-team::mcp::bridge，供 dream-core-ai-agent 反序列化 AcpBuildExtra 用)
...
**废弃**：`dream-core-team::mcp::bridge::TeamMcpStdioConfig` 原定义改成 `pub use dream_core_api_types::TeamMcpStdioConfig;`（保持 import path 不断）。

**不引入依赖**：`dream-core-api-types` 继续禁 axum/tower（AGENTS.md 硬规则）。

---

## 2. `dream-core-ai-agent::types::AcpBuildExtra` 扩展（Wave 1 · 模块 D2）

**文件**：`crates/dream-core-ai-agent/src/types.rs`

```rust
use dream_core_api_types::TeamMcpStdioConfig;
...
- **字段命名规则**：新增字段统一 **snake_case**（本字段 `team_mcp_stdio_config` 即如此）；既有 `teamId` 字段（[`service.rs:73`](../../../crates/dream-core-team/src/service.rs) 早期写入）**留作历史兼容**不动，但不再新增驼峰字段。conversation.extra 的 JSON 由后端自己写自己读，snake_case 自洽即可（commit `dae96f8` 方向）

---

## 3. `dream-core-team::mcp::bridge` 新增 ServerSpec（Wave 1 · 模块 D3）

**文件**：`crates/dream-core-team/src/mcp/bridge.rs`
...
## 4. `dream-core-team::mcp::tools` 新增两个工具 descriptor（Wave 1 · 模块 D4）

**文件**：`crates/dream-core-team/src/mcp/tools.rs`
...
## 5. `dream-core-team::prompts` 大幅扩写（Wave 1 · 模块 D5）
...
## 6. `dream-core-team::session::TeamSession` 新方法（Wave 2 · 模块 D7）

**文件**：`crates/dream-core-team/src/session.rs`
...
## 6.5 `dream-core-team::scheduler::TeammateManager` 签名扩展（Wave 2 · 模块 D8）
...
## 7. `dream-core-ai-agent::acp_agent::session_new_and_prompt` 注入（Wave 2 · 模块 D10）

**文件**：`crates/dream-core-ai-agent/src/acp_agent.rs:448-458`
...
## 8. `dream-core-app` 新增子命令 `mcp-bridge`（Wave 1 · 模块 D6）

**文件**：`crates/dream-core-app/src/lib.rs`（或新子模块 `bridge.rs`）

**CLI 入口**：
```rust
// main.rs
if args.get(1).map(|s| s.as_str()) == Some("mcp-bridge") {
    dream_core_app::bridge::run_mcp_bridge().await;
    return;
}
```
...
/// 3. TCP 连 127.0.0.1:<port>：dream_core_team::mcp::protocol 的 4 字节长度帧 + JSON-RPC
...
## 9. `dream-core-team::service::TeamSessionService` 签名扩展（Wave 2 · 模块 D9）

**文件**：`crates/dream-core-team/src/service.rs`
...
## 10. `dream-core-app::state_builders::build_team_state` 扩展（Wave 2 · 模块 D11）

**文件**：`crates/dream-core-app/src/state_builders.rs:304`
...
`backend_binary_path` 在 `dream_core_app::lib::build_router` 一次性 `Arc::new(std::env::current_exe()?)` 后 clone 到各子 builder。

---

## 11. ConversationService 不改（关键决策）

**phase1 决定不改 `dream-core-conversation::service::build_task_options`**。
...
```rust
// crates/dream-core-db/src/repository/team.rs
...
**Routes 改动**：`crates/dream-core-team/src/routes.rs` 所有 handler 从 `Extension<AuthUser>` 取 `user_id` 传给 service。
...
**文件**：`crates/dream-core-api-types/src/team.rs`
...
**新 trait**：放 `dream-core-conversation` 里（避免 `dream-core-conversation` 反向依赖 `dream-core-team`）：

```rust
// crates/dream-core-conversation/src/service.rs（或新 traits.rs）
#[async_trait]
...
**文件**：`crates/dream-core-common/src/lib.rs`
...
**文件**：`crates/dream-core-team/src/mcp/protocol.rs`

```rust
// 原：const MAX_MCP_MESSAGE_SIZE: usize = 10 * 1024 * 1024;
// 改：
pub const MAX_MCP_MESSAGE_SIZE: usize = dream_core_common::TEAM_MCP_MAX_FRAME_BYTES;
```

**文件**：`crates/dream-core-team/src/mcp/server.rs`

```rust
// dispatch_tool 外层：
let result = tokio::time::timeout(
    std::time::Duration::from_millis(dream_core_common::TEAM_MCP_REQUEST_TIMEOUT_MS),
    dispatch_tool(...),
).await;
...
**文件**：`crates/dream-core-ai-agent/src/task_manager.rs`
...
**文件**：`crates/dream-core-ai-agent/src/acp_agent.rs`
...
impl TeamSession {
    pub async fn on_agent_finish(
        ...
        if let AgentStreamChunk::Error { message } = &chunk {
            if dream_core_common::RATE_LIMIT_REGEX.is_match(message) {
...
**文件**：`crates/dream-core-app/src/bridge.rs`（D6 已实现 bridge 主逻辑；W4-D24 在 TCP connect 成功 + initialize ok 后追加）
...
**文件**：`crates/dream-core-team/src/guide/server.rs`（新增）
...
**文件**：`crates/dream-core-app/src/bridge.rs`（在 D6 已有的 `run_mcp_bridge()` 内部分叉）

```rust
pub async fn run_mcp_bridge() -> ! {
    if std::env::var(dream_core_team::guide::GuideStdioConfig::ENV_BACKEND).is_ok() {
...
**文件**：`crates/dream-core-ai-agent/src/acp_agent.rs`（instructions 构造点）
