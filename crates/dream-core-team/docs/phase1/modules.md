...
### D1 — `dream-core-api-types::team_mcp` 子模块

| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-api-types/src/team_mcp.rs`（新增） + `lib.rs` 导出（修改） |
...
| 预估 LoC | 40 行 |
| 预估人天 | 0.5 |
| 接口契约 | [interface-contracts.md §1](./interface-contracts.md#1-dream-core-api-types-新增类型wave-1--模块-d1) |

---

### D2 — `dream-core-ai-agent::AcpBuildExtra` 字段扩展

| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-ai-agent/src/types.rs`（修改，只加 1 个字段 + import） |
| 职责 | 加 `#[serde(default)] team_mcp_stdio_config: Option<TeamMcpStdioConfig>` |
| 输入 | D1 导出的类型 |
| 输出 | 新字段可反序列化 |
| 依赖 | D1 必须先 merge（Cargo 依赖层面）**— phase1 特例：D2 里先用 `pub use dream_core_api_types::TeamMcpStdioConfig;`，单元测试里用 stub struct 跑通 JSON；待 D1 merge 后只需删 stub** |
| 测试策略 | 2 条：旧 JSON（无字段）反序列化为 None；新 JSON 正确解出 config |
| 预估 LoC | 15 行 |
| 预估人天 | 0.3 |
| 接口契约 | [§2](./interface-contracts.md#2-dream-core-ai-agenttypesacpbuildextra-扩展wave-1--模块-d2) |

---

### D3 — `dream-core-team::mcp::bridge::TeamMcpStdioServerSpec`

| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/mcp/bridge.rs`（修改：替换原 `TeamMcpStdioConfig` 用 `pub use` + 新增 ServerSpec） |
...
| 接口契约 | [§3](./interface-contracts.md#3-dream-core-teammcpbridge-新增serverspecwave-1--模块-d3) |
...
### D5 — `dream-core-team::prompts` 三层模板 + builder
...
### D6 — `dream-core-app mcp-bridge` 子命令

| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-app/src/bridge.rs`（新增） + `main.rs`（新增 argv 分支） |
| 职责 | 实现 stdio↔TCP 透传（mcp.md §4.6 的 4 步） |
| 依赖 | `rmcp` 或手写最小 JSON-RPC 2.0 over stdio；`dream-core-team::mcp::protocol::{read_frame, write_frame}`；D1 的 env key |
...
| 预估人天 | 1.5 |
| 接口契约 | [§8](./interface-contracts.md#8-dream-core-app-新增子命令-mcp-bridgewave-1--模块-d6) |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/session.rs`（只新增三个 pub 方法，不改 send 路径） |
...
| 预估 LoC | 150 |
| 预估人天 | 1.5 |
| 接口契约 | [§6](./interface-contracts.md#6-dream-core-teamsessionteamsession-新方法wave-2--模块-d7) |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/session.rs`（只改两个 send 方法）+ `crates/dream-core-api-types/src/team.rs`（请求 DTO 加 `files: Option<Vec<String>>` 字段）+ `crates/dream-core-team/src/routes.rs`（handler 透传 files） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/service.rs`（修改 `new` + `ensure_session`） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-ai-agent/src/acp_agent.rs`（修改 `session_new_and_prompt` + 构造函数加 `backend_binary_path` 字段） |
| 职责 | 按 [interface-contracts.md §7](./interface-contracts.md#7-dream-core-ai-agentacp_agentsession_new_and_prompt-注入wave-2--模块-d10) 改造 `NewSessionRequest` 构造 |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-app/src/state_builders.rs`（修改 `build_team_state` 签名）+ `lib.rs`（传 `backend_binary_path`）+ `crates/dream-core-app/tests/team_phase1_smoke.rs`（新增） |
| 职责 | 1) `build_team_state` 按 [interface-contracts.md §10](./interface-contracts.md#10-dream-core-appstate_buildersbuild_team_state-扩展wave-2--模块-d11) 加参数<br>2) `lib.rs::build_router` 一次性 `current_exe()` 缓存到 `Arc<PathBuf>`<br>3) e2e smoke：按 [README.md §3](./README.md) 的 8 步脚本实跑，校验 agent 能调 `team_send_message` |
| 输入 | D7 / D9 / D10 |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/service.rs`（仅 `remove_team` 方法内在 `stop_session` 前循环遍历 `team.agents` 调 `task_manager.kill(conv_id, Some(AgentKillReason::TeamDeleted))`） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/service.rs`（仅 `list_teams`）+ `crates/dream-core-db/src/repository/team.rs`（加 `list_by_user`）+ `crates/dream-core-team/src/routes.rs`（仅 list handler） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/service.rs`（仅 `get_team` 签名改造）+ `crates/dream-core-team/src/routes.rs`（仅 get handler） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/service.rs`（仅 `remove_team`）+ `crates/dream-core-team/src/routes.rs`（仅 delete handler） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-db/src/repository/conversation.rs`（trait 加方法）+ `crates/dream-core-db/src/repository/sqlite_conversation.rs`（实现） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/service.rs`（新增私有 fn，不触碰 `get_team`） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/service.rs`（仅在 `get_team` 末尾加 if 分支） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/scheduler.rs` 新子模块（或 `utils.rs`），暴露 `pub fn normalize_name(&str) -> String` |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/scheduler.rs`（`TeammateManager::rename_agent` 改造 + 内存字段 `renamed_agents: Mutex<HashMap<String, String>>`） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/prompts/lead.rs` + `teammate.rs`（只改 `## Your Teammates` 段的渲染逻辑） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-api-types/src/team.rs`（`CreateAgentRequest` 加字段） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/service.rs`（`create_team` 内 for each agent 循环里加一个 if 分支） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-conversation/src/traits.rs`（新增 trait）+ `crates/dream-core-conversation/src/state.rs`（`ConversationService` 加 `team_router: Option<Arc<dyn ITeamMessageRouter>>`） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-conversation/src/service.rs`（仅 `send_message` 入口增加一段 if） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/service.rs`（`impl ITeamMessageRouter for TeamSessionService`）+ `crates/dream-core-app/src/state_builders.rs`（装配时把 team_session_service clone 作为 router 传进 conversation state） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-common/src/lib.rs`（新增常量 `TEAM_MCP_MAX_FRAME_BYTES`）+ `crates/dream-core-team/src/mcp/protocol.rs`（`MAX_MCP_MESSAGE_SIZE` 改为引用） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-common/src/lib.rs`（`TEAM_MCP_REQUEST_TIMEOUT_MS`）+ `crates/dream-core-team/src/mcp/server.rs`（dispatch_tool 外层 `tokio::time::timeout`） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-ai-agent/src/types.rs`（新 enum） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-ai-agent/src/task_manager.rs`（trait 加方法） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-ai-agent/src/acp_agent.rs`（只加字段 `stream_tx: broadcast::Sender<AgentStreamChunk>` + 构造函数初始化 capacity 256 + impl `AgentManagerHandle::subscribe_stream`） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-ai-agent/src/acp_agent.rs`（现有 5 个 chunk 处理点各插一行 `let _ = self.stream_tx.send(...)`） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/scheduler.rs`（`TeammateManager` 加字段 `active_wakes: DashSet<String>` + 两个方法 `try_acquire_wake_lock` / `release_wake_lock`） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/scheduler.rs`（`TeammateManager` 只加字段 `wake_timeouts: DashMap<String, JoinHandle<()>>` + `clear_wake_timeout(slot_id)` 实现） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/scheduler.rs`（`arm_wake_timeout(slot_id, stream_rx)` 方法 spawn 后台 task） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/session.rs`（`compute_wake_input` / `send_message` 开头结尾加 acquire / release） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/scheduler.rs`（`TeammateManager` 加字段 `finalized_turns: DashMap<String, Instant>` + `begin_finalize(conv_id) -> bool` + `clear_finalized_turn(conv_id)`） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/session.rs`（`on_agent_finish` 第一行插入 `if !scheduler.begin_finalize(conv_id) { return; }`；wake 成功后调 `clear_finalized_turn`） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/scheduler.rs`（新 fn + `CrashReason` enum） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/scheduler.rs`（`handle_agent_crash` 内部"写 testament"私有 helper） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/scheduler.rs`（`handle_agent_crash` 主体：串接 D20b-1 的 testament + 下列"进程卫生"步骤） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-common/src/lib.rs`（`RATE_LIMIT_REGEX` once_cell Lazy）+ `crates/dream-core-team/src/session.rs`（`on_agent_finish` 里 Error chunk 分支：若 regex 命中 → `set_status(Failed)` 不走 crash） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/scheduler.rs`（`handle_inactivity_timeout(slot_id)` 被 D18b 的 timer 调用） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/service.rs`（`TeamSessionService` 加 `add_agent_locks: DashMap<String, Arc<tokio::sync::Mutex<()>>>` + `add_agent` 入口取锁 + `remove_team` 清 lock entry） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/mcp/protocol.rs`（新枚举 `McpNotification::McpReady { slot_id, auth_token }`） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/mcp/server.rs`（`TeamMcpServer` 加字段 `ready_latch: DashSet<String>` + `ready_notify: DashMap<String, Arc<Notify>>`） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/mcp/server.rs`（方法实现：收到 D24a 的通知帧时调） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-app/src/bridge.rs`（D6 主 bridge 逻辑内 initialize 成功后追加一行 send） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/guide/server.rs`（新 struct + `start_singleton` + `stop`） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/guide/handlers.rs`（新 `parse_create_team_args(args, caller_conversation) -> CreateTeamParams` 纯函数） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/guide/handlers.rs`（handler 主体） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/guide/handlers.rs`（新 fn，仅 list_models） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-ai-agent/src/acp_agent.rs`（构造 instructions 的地方加分支） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-ai-agent/src/acp_agent.rs`（`session_new_and_prompt` 里 mcp_servers 构造的地方加分支） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/session.rs`（新 struct `SpawnAgentRequest` + `TeamSession::spawn_agent` 空壳 fn 签名，body `todo!()`） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/session.rs`（`spawn_agent` 第一段校验） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/session.rs`（`spawn_agent` 第二段校验） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/session.rs`（`spawn_agent` 第三段校验） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/service.rs`（`add_agent` 扩展：原签名保留，内部持 W4-D23 lock + build conversation + 分配 slot_id + update repo） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/session.rs`（`spawn_agent` 中段第 1 步） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/session.rs`（`spawn_agent` 中段第 2/3 步） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/session.rs`（`spawn_agent` 末段第 1 步） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/session.rs`（`spawn_agent` 末段第 2 步） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/session.rs`（`spawn_agent` 末段第 3 步） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/mcp/server.rs`（`handle_send_message` 顶部加 if 分支） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/mcp/server.rs`（在 D30a-1 识别后调用的 async helper） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/mcp/server.rs`（同 D30a 所在位置的第二个拦截分支） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/scheduler.rs`（`shutdown_agent` 方法顶部） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/scheduler.rs`（`remove_agent` 第 1 步） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/scheduler.rs`（`remove_agent` 第 2 步；使用 W4-D18a/b-1/W4-D19a 已有 API） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/scheduler.rs`（`remove_agent` 第 3 步） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-api-types/src/team.rs`（新 enum + 两个 payload struct） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/mcp/server.rs`（在 TCP `bind` 成功 / 失败处各插一行 broadcast） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/service.rs`（`ensure_session` 的 6 个分支处各插 broadcast） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-app/src/bridge.rs`（bridge 的 tools/list 前 / 后两个时机） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/dream-core-team/src/session.rs`（`compute_wake_input` 内 teammate 分支末尾遍历 unread_messages 逐条 emit） |
...
|:---:|:---:|---|---|:-:|:---:|
| D1 | 1 | team_mcp types | `dream-core-api-types/src/team_mcp.rs` | 40 | 0.5 |
| D2 | 1 | AcpBuildExtra 字段 | `dream-core-ai-agent/src/types.rs` | 15 | 0.3 |
| D3 | 1 | Stdio ServerSpec | `dream-core-team/src/mcp/bridge.rs` | 80 | 1.0 |
| D4 | 1 | 两个新 MCP 工具（list_models / describe_assistant） | `dream-core-team/src/mcp/{tools,server}.rs` | 150 | 1.0 |
| **D4b** | 1 | **`TEAM_SPAWN_AGENT_DESCRIPTION` 原文常量**（P0#48 补漏） | `dream-core-team/src/mcp/tools.rs` | 40 | 0.3 |
| D5a | 1 | Team Guide Prompt | `dream-core-team/src/prompts/team_guide.rs` | 120 | 0.5 |
| D5b-1 | 1 | Lead Prompt 常量 (include_str! txt) | `lead.rs` + `prompt_templates/lead.txt` | ⚠️ 48 rust+188 txt | 0.3 |
| D5b-2 | 1 | Lead Prompt builder | `dream-core-team/src/prompts/lead.rs` | 30 | 0.5 |
| D5c | 1 | Teammate Prompt + wake payload | `dream-core-team/src/prompts/teammate.rs` | ⚠️ 150 | 1.0 |
| D6 | 1 | mcp-bridge subcommand | `dream-core-app/src/bridge.rs` | 180 | 1.5 |
| **D7a** | 2 | **TeamSession 三个新方法（compute/spec/finish）** | `dream-core-team/src/session.rs` | 150 | 1.5 |
| **D7b** | 2 | **send 路径接 wake + `files` 附件 + log-not-throw**（P0#45/#46） | `dream-core-team/src/session.rs` + `dream-core-api-types/src/team.rs` + `routes.rs` | 120 | 1.2 |
| **D7c** | 2 | **`send_message_to_agent(silent=true)` 占位** | `dream-core-team/src/session.rs` | 40 | 0.3 |
| D8 | 2 | Scheduler 首次 wake | `dream-core-team/src/scheduler.rs` | 120 | 1.5 |
| D9 | 2 | ensure_session 闭环 | `dream-core-team/src/service.rs` | ⚠️ 200 | 2.0 |
| D10 | 2 | acp_agent 注入 | `dream-core-ai-agent/src/acp_agent.rs` | 60 | 1.0 |
| D11 | 2 | app 装配 + smoke test | `dream-core-app/*` | 180 | 2.0 |
| **D11.5** | 2 | **`remove_team` 级联 kill agent 进程**（P0#47 补漏） | `dream-core-team/src/service.rs` | 40 | 0.3 |
