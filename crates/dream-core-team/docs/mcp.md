...
- 参见 `crates/dream-core-team/src/mcp/protocol.rs` 的 `read_frame` / `write_frame`
...
```json
{
  ...
  "result": {
    ...
    "serverInfo": { "name": "dream-core-team-mcp", "version": "1.0.0" },
    "capabilities": { "tools": {} }
  }
}
```
...
| 管理模块 | `dream-core-mcp` crate（`build_session_mcp_servers()`） | `dream-core-team` crate（`TeamMcpStdioConfig`） |
...
| 操作 | 现有 API | 位置 |
|------|----------|------|
| 杀旧进程 | `IWorkerTaskManager::kill(conv_id, reason)` | `crates/dream-core-ai-agent/src/task_manager.rs` |
...
| 文件 | 改动 | 影响范围 |
|------|------|----------|
| `dream-core-ai-agent/src/types.rs` | `AcpBuildExtra` 加 `#[serde(default)] team_mcp_stdio_config: Option<TeamMcpStdioConfig>` | 旧 extra 无此字段 → `None`，零影响 |
| `dream-core-ai-agent/src/acp_agent.rs :: session_new()` | 如果 `team_mcp_stdio_config.is_some()`，构造 `mcpServers` 数组追加到 payload | 单聊 config 为 `None` → 不追加，零影响 |
| `dream-core-mcp/src/session_injection.rs` | team MCP 作为额外一项 append 到 `build_session_mcp_servers()` 结果里（或在 `session_new` 里直接 merge） | 视实现选择 |
...
`crates/dream-core-team/src/mcp/bridge.rs`：
...
| 内容 | 位置 |
|------|------|
| TCP server 启停 | `crates/dream-core-team/src/mcp/server.rs :: TeamMcpServer` |
| 握手 + 鉴权 | `crates/dream-core-team/src/mcp/server.rs :: handle_initialize` |
| Tool dispatch | `crates/dream-core-team/src/mcp/server.rs :: dispatch_tool` |
| Tool descriptor（tools/list 返回） | `crates/dream-core-team/src/mcp/tools.rs :: all_tool_descriptors` |
| Backend 白名单 | `crates/dream-core-team/src/mcp/tools.rs :: SPAWN_BACKEND_WHITELIST` |
| 帧 + JSON-RPC 类型 | `crates/dream-core-team/src/mcp/protocol.rs` |
| Stdio config 结构 | `crates/dream-core-team/src/mcp/bridge.rs :: TeamMcpStdioConfig` |
| Session 与 server 生命周期绑定 | `crates/dream-core-team/src/session.rs :: TeamSession::start` |
| Scheduler 对 MCP action 的执行 | `crates/dream-core-team/src/scheduler.rs :: execute_action` |
| `SpawnAgent` 空壳位置 | `crates/dream-core-team/src/scheduler.rs`（搜 `"spawn_agent action — requires TeamSession to complete"`）|
| Mailbox 写入 | `crates/dream-core-team/src/scheduler.rs :: handle_send_message` |
