...
impl TeamMcpStdioServerSpec {
    ...
    ///
    /// `backend_binary_path` is the absolute path to the `dreamcore`
...
mod tests {
    ...
    fn sample_cfg() -> TeamMcpStdioConfig {
        TeamMcpStdioConfig {
            ...
            token: "tok-abc".into(),
            slot_id: "slot-1".into(),
            binary_path: "/usr/bin/dreamcore".into(),
        }
    }

    #[test]
    fn from_config_fills_all_fields() {
        let spec = TeamMcpStdioServerSpec::from_config("/usr/bin/dreamcore", &sample_cfg());

        assert_eq!(spec.name, TEAM_MCP_SERVER_NAME);
        assert_eq!(spec.command, "/usr/bin/dreamcore");
    ...
    fn into_sdk_serializes_as_stdio_variant() {
        let spec = TeamMcpStdioServerSpec::from_config("/bin/dreamcore", &sample_cfg());
        ...
        assert_eq!(json["command"], "/bin/dreamcore");
