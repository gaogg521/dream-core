...
impl TeamAgentProvisioner {
    ...
    async fn resolve_team_tool_transport(
        ...
        user_id: &str,
        agent: &TeamAgent,
    ) -> Result<(TeamToolTransport, dream_core_common::ResolvedBackendCapabilities), TeamError> {
    ...
    async fn create_team_conversation_for_agent(
        ...
        let provider_id = if agent_type == AgentType::DreamEngine {
        ...
        let (top_level_model, extra) = if agent_type == AgentType::DreamEngine {
            (
    ...
    fn build_team_extra(
        ...
        if agent_type != AgentType::DreamEngine {
            extra["current_model_id"] = serde_json::Value::String(model.to_owned());
        }
        if let Some(assistant_id) = assistant_id {
...
mod tests {
    ...
    use dream_core_ai_agent::{AgentError, AgentInstance};
    ...
    impl TeamToolCapabilityPort for TestCapabilityPort {
        async fn resolve(
            ...
            Ok(dream_core_common::ResolvedBackendCapabilities {
                ...
                origin: if direct {
                    dream_core_common::CapabilityOrigin::DirectDescriptor
                } else if internal {
                    dream_core_common::CapabilityOrigin::InternalDescriptor
                } else {
                    dream_core_common::CapabilityOrigin::Unknown
                },
            })
        }
    }
