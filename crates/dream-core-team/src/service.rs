...
use dream_core_common::{AgentKillReason, ConversationStatus, TimestampMs, generate_id, now_ms};
...
impl TeamSessionService {
    ...
    pub async fn get_run_state(&self, user_id: &str, team_id: &str) -> Result<TeamRunStateResponse, TeamError> {
        ...
        let active_run = session.team_run_manager().current_payload(&snapshot).filter(|run| {
            matches!(
                run.status,
                dream_core_api_types::TeamRunStatus::Accepted
                    | dream_core_api_types::TeamRunStatus::Running
                    | dream_core_api_types::TeamRunStatus::Cancelling
            )
        });
    ...
    pub async fn resolve_team_tool_context(
        ...
        let allowed_tools = dream_core_api_types::team_tool_descriptors_for_role(role_to_tool_role(agent.role))
            .into_iter()
...
async fn set_active_agent_session_mode(instance: &AgentInstance, mode: &str) -> Result<(), AgentError> {
    ...
    match instance {
        ...
        AgentInstance::DreamEngine(manager) => manager.set_mode(mode).await,
...
mod tests {
    ...
    use dream_core_common::{AgentKillReason, AgentType, ConversationStatus, TimestampMs, now_ms};
    use dream_core_db::{IConversationRepository, ITeamRepository};
    ...
    fn two_agent_team_request(name: &str) -> dream_core_api_types::CreateTeamRequest {
        dream_core_api_types::CreateTeamRequest {
            name: name.into(),
            agents: vec![
                dream_core_api_types::TeamAgentInput {
                    name: "Lead".into(),
                    role: "lead".into(),
    ...
    async fn clear_agent_context_resets_anchor_and_runtime_state_without_membership_noise() {
        ...
        let notice: dream_core_api_types::TeamContextResetNotice = serde_json::from_str(
            notices
                .last()
