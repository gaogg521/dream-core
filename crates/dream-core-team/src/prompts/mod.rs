...
use dream_core_api_types::TeamToolTransport;

mod wake_summary;

pub use dream_core_team_prompts::AvailableAssistant;

use crate::types::{MailboxMessage, MailboxMessageType, TeamAgent, TeamTask};

fn to_prompt_role(role: crate::types::TeammateRole) -> dream_core_team_prompts::TeamPromptRole {
    match role {
        crate::types::TeammateRole::Lead => dream_core_team_prompts::TeamPromptRole::Lead,
        crate::types::TeammateRole::Teammate => dream_core_team_prompts::TeamPromptRole::Teammate,
    }
}

fn to_prompt_agent(agent: &TeamAgent) -> dream_core_team_prompts::TeamPromptAgent {
    dream_core_team_prompts::TeamPromptAgent {
...
pub fn build_lead_prompt_for_transport(
    ...
    let body = dream_core_team_prompts::build_lead_prompt(&dream_core_team_prompts::LeadPromptParams {
        agent: &prompt_agent,
        team_name,
...
pub fn build_teammate_prompt_for_transport(
    ...
    let leader = prompt_members
        .iter()
        .find(|candidate| candidate.role == dream_core_team_prompts::TeamPromptRole::Lead)
        .cloned()
        .unwrap_or_else(|| dream_core_team_prompts::TeamPromptAgent {
            slot_id: "lead".to_owned(),
            name: "Lead".to_owned(),
            role: dream_core_team_prompts::TeamPromptRole::Lead,
            backend: agent.backend.clone(),
            model: agent.model.clone(),
            status: None,
        });
    ...
    dream_core_team_prompts::build_teammate_prompt(&dream_core_team_prompts::TeammatePromptParams {
        agent: &prompt_agent,
        team_name,
        leader: &leader,
        teammates: &teammates,
