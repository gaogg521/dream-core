...
use dream_core_common::{AgentKillReason, generate_id};
...
impl TeamSession {
    ...
    pub async fn start_with_prompt_dump(
        ...
        let auth_token = dream_core_common::generate_id();
...
mod tests {
    ...
    impl TeamProjectionMessageStore for NoopProjectionStore {
        ...
        async fn find_projected_message(
            ...
            _msg_id: &str,
            _msg_type: &str,
        ) -> Result<Option<dream_core_db::models::MessageRow>, TeamError> {
            Ok(None)
        }

        async fn insert_projected_message(&self, _row: &dream_core_db::models::MessageRow) -> Result<(), TeamError> {
            Ok(())
        }
    }
    ...
    async fn shutdown_without_active_run_opens_system_lifecycle_run() {
        ...
        assert_eq!(payload.source, dream_core_api_types::TeamRunSource::SystemLifecycle);
    ...
    async fn message_to_runtime_starting_member_returns_blocked_runtime_starting() {
        ...
        assert_eq!(
            ...
            Some(dream_core_api_types::TeamSlotBlockedReason::RuntimeStarting)
        );
        session.stop();
    }
