...
use dream_core_db::models::AgentMetadataRow;
use dream_core_db::{IAgentMetadataRepository, resolve_agent_binding_from_rows};
use std::sync::Arc;
...
mod tests {
    ...
    use dream_core_db::models::{AgentMetadataRow, AssistantDefinitionRow, AssistantOverlayRow, Provider};
    use dream_core_db::{
    ...
    impl IProviderRepository for SingleProviderRepo {
        ...
        async fn create(&self, _params: dream_core_db::CreateProviderParams<'_>) -> Result<Provider, DbError> {
    ...
    fn parse_agent_type_accepts_top_level_supported_runtimes() {
        ...
        assert_eq!(parse_agent_type("aionrs").unwrap(), AgentType::DreamEngine);
    }
    ...
    fn a_found_row_types_the_agent_instead_of_assuming_acp() {
        ...
        assert_eq!(agent_type_for_backend(None, "aionrs").unwrap(), AgentType::DreamEngine);
    }
    ...
    async fn add_agent_rejects_assistant_missing_from_team_selectable_catalog() {
        ...
        let err = svc
            .add_agent(
                "user1",
                &created.id,
                dream_core_api_types::AddAgentRequest {
                    name: "Worker".into(),
                    role: "teammate".into(),
                    backend: None,
