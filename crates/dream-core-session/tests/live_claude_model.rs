...
//!
//! ```text
//! cargo test -p dream-core-session --test live_claude_model -- --ignored --nocapture
//! ```
//!
...
async fn observe_run(model: Option<&str>) -> Option<(String, String)> {
    ...
    let spawner: Arc<dyn Spawner> = Arc::new(dream_core_process::RealSpawner::new(
        Arc::new(dream_core_process::FileRegistryStore::new(tmp.path())),
        uuid::Uuid::now_v7(),
        "live-test",
    ));
...
/// The model Dream UI asks for is the model claude runs — end to end, through the real
