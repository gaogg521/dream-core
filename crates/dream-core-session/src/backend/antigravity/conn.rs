...
pub struct AntigravitySessionBackend {
    ...
    current: Arc<Mutex<Option<Arc<dream_core_process::ManagedProcess>>>>,
...
impl AntigravitySessionBackend {
    ...
    async fn start_turn(&self, content: Vec<ContentBlock>) -> Result<u64, BackendError> {
        ...
        if let Some(cwd) = self.config.cwd.as_deref() {
            ...
            spawn_env.push(dream_core_common::EnvVar {
                name: "PWD".to_owned(),
                value: cwd.to_owned(),
            });
        }
    ...
    fn spawn_reader(&self, proc: Arc<dream_core_process::ManagedProcess>, turn_gen: u64) {
