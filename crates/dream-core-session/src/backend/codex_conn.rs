...
use dream_core_process::Spawner;
...
impl BackendConnection for CodexConnection {
    async fn open_session(
        ...
        let cmd = dream_core_common::CommandSpec {
...
fn initialize_params() -> HandshakeParams {
    HandshakeParams(json!({
        "clientInfo": { "name": "dream-core-session", "version": "0.1.0" },
...
pub struct CodexSessionBackend {
    ...
    stdin: Arc<Mutex<Option<dream_core_process::BoxedStdin>>>,
...
fn start_codex_reader(
    ...
    stdout: Option<dream_core_process::BoxedStdout>,
    io: Arc<dyn AgentIo>,
...
async fn handle_reverse_rpc(
    ...
    match method {
        ...
        _ => {
            write_reverse_error(stdin, &id, -32601, "method not handled by dream-core-session").await;
            emit(
                event_tx,
                session_id,
                cur,
        ...
        match command {
            Command::Send { content, metadata } => {
                ...
                if matches!(route, Some(SlashRoute::Logout)) {
                    self.suspend
                        .ensure_awake(dream_core_common::now_ms(), || self.wake_handle())
                        .await?;
...
mod tests {
    ...
    fn command_execution_uses_codex_semantic_action_as_its_label() {
        ...
        let list = serde_json::json!({
            "type": "commandExecution",
            "commandActions": [{ "type": "listFiles", "path": "crates/dream-core-session" }]
        });
        assert_eq!(
            command_execution_display_name(&list),
            "List files crates/dream-core-session"
        );
    }
    ...
    fn b1_initialize_frame_opts_into_experimental_api_nested() {
        ...
        assert_eq!(frame["params"]["clientInfo"]["name"], "dream-core-session");
    }
    ...
    async fn open_session_classifies_workspace_unavailable_spawn_failure() {
        ...
        #[async_trait::async_trait]
        impl dream_core_process::Spawner for WorkspaceGoneSpawner {
            async fn spawn(
                &self,
    ...
    async fn f4_suspend_then_wake_respawns_through_spawner() {
        ...
        assert!(
            backend
                .suspend
                .suspend_if_idle(dream_core_common::now_ms() + 10_000, false)
                .await
        );
