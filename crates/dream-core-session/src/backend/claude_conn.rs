...
/// Pure `serde_json`, no ACP SDK — `dream-core-session` stays SDK-free.
...
struct ClaudeWakeRecipe {
    ...
    env: Vec<dream_core_common::EnvVar>,
                        ...
                        match tokio::time::timeout(std::time::Duration::from_millis(200), stdout.read(&mut chunk)).await {
                            ...
                            Ok(Ok(n)) if n > 0 => {
                                ...
                                match parsed {
                                    ...
                                    Err(_) => {
                                        tracing::error!(
                                            target: "dream_core_session::backend::claude_conn",
                                            conversation_id = %session_id,
...
mod tests {
    ...
    async fn f4_turn_in_flight_blocks_idle_suspend() {
        ...
        let suspended = backend
            .suspend
            .suspend_if_idle(dream_core_common::now_ms() + 10_000, true)
            .await;
