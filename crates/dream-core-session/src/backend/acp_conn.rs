...
pub struct AcpConnection {
    ...
    command: dream_core_common::CommandSpec,
}

impl AcpConnection {
    pub fn new(spawner: Arc<dyn Spawner>, command: dream_core_common::CommandSpec) -> Self {
...
struct AcpWakeRecipe {
    ...
    command: Option<dream_core_common::CommandSpec>,
    config: SessionConfig,
}
...
fn enrich_acp_error_message(raw: &str, stderr_tail: Option<&str>) -> String {
    ...
    if let Some(tail) = stderr_tail
        && let Some(cause) = dream_core_common::error_extract::extract_error_message(tail)
    {
...
mod tests {
    ...
    async fn f4_suspend_then_wake_respawns_through_spawner() {
        ...
        let command = dream_core_common::CommandSpec {
            command: "hermes".into(),
            args: vec!["acp".into()],
            env: Vec::new(),
            cwd: None,
