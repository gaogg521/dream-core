...
async fn write_frame(stdin: &mut dream_core_process::BoxedStdin, frame: &Value) -> Option<()> {
...
impl SpawnedTitleIo {
    pub(crate) fn new(spawner: Arc<dyn dream_core_process::Spawner>, command: dream_core_common::CommandSpec) -> Self {
...
impl TitleIoSource for SpawnedTitleIo {
    async fn open(&self) -> Option<Box<dyn AgentIo>> {
        match self
            .spawner
            .spawn(self.command.clone(), &[], "dream-core-session-title")
            .await
        {
