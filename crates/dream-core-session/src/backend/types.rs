...
impl BackendError {
    ...
    /// before.
    pub(crate) fn from_spawn(context: &'static str, e: dream_core_process::ProcessError) -> Self {
        match e {
            dream_core_process::ProcessError::WorkspaceUnavailable(path) => Self::WorkspaceUnavailable(path),
...
mod tests {
    ...
    fn from_spawn_keeps_workspace_unavailable_class() {
        let err = BackendError::from_spawn(
            "codex spawn failed",
            dream_core_process::ProcessError::workspace_unavailable("/gone/workspace"),
        );
        assert_eq!(err, BackendError::WorkspaceUnavailable("/gone/workspace".into()));

        let err = BackendError::from_spawn("codex spawn failed", dream_core_process::ProcessError::internal("boom"));
        assert_eq!(
            err,
