...
use dream_core_process::{BoxedStdin, ProcessError, Spawner};
...
impl BackendAdapter for ClaudeAdapter {
    async fn start_turn(
        ...
        let proc = spawner.spawn(spec, &[], "dream-core-session").await?;
...
mod tests {
    ...
    async fn captured_frame_with_id(content: &[crate::ContentBlock], client_msg_id: Option<&str>) -> String {
        ...
        let mut stdin: dream_core_process::BoxedStdin = Box::new(CaptureStdin(buf.clone()));
        ClaudeAdapter::new()
    ...
    fn tool_call_label_prefers_claudes_own_description() {
        ...
        assert_eq!(
            tool_call_display_name(
                "Bash",
                &serde_json::json!({"command": "cargo test -p dream-core-session", "description": "Re-run format tests"})
            ),
            "Re-run format tests"
        );
