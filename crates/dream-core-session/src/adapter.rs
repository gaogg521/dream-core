...
use dream_core_process::{BoxedStdin, BoxedStdout, ManagedProcess, ProcessError, Spawner};
...
/// `None` when nothing matches the `dream_core_common::error_extract` allowlist
...
pub(crate) async fn redact_exit_stderr(io: &dyn AgentIo) -> Option<String> {
    ...
    dream_core_common::error_extract::extract_error_message(&tail)
}
