...
use dream_core_common::{generate_id, now_ms};
use dream_core_db::models::AssistantUserRow;
use dream_core_db::{IChannelRepository, SqliteChannelRepository, init_database_memory};
use dream_core_realtime::EventBroadcaster;

use dream_core_channel::action::{ActionExecutor, MessageResult};
use dream_core_channel::channel_settings::ChannelSettingsService;
use dream_core_channel::pairing::PairingService;
use dream_core_channel::session::SessionManager;
use dream_core_channel::types::{
...
async fn setup() -> (
    ...
    let pref_repo: Arc<dyn dream_core_db::IClientPreferenceRepository> =
        Arc::new(dream_core_db::SqliteClientPreferenceRepository::new(db.pool().clone()));
