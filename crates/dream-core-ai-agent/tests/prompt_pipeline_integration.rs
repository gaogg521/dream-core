...
use dream_core_ai_agent::capability::AcpVisionPolicy;
use dream_core_ai_agent::capability::prompt_pipeline::{PromptCtx, PromptPipeline};
use dream_core_ai_agent::factory::acp_assembler::{AcpSessionParams, WorkspaceInfo, assemble_acp_params};
use dream_core_ai_agent::manager::acp::{AcpSession, ImageAttachmentVisionHook, SessionNewPreludeHook};
use dream_core_ai_agent::registry::AgentRegistry;
use dream_core_ai_agent::shared_kernel::ModelId;
use dream_core_ai_agent::{AcpBuildExtra, AcpSkillManager, AgentRuntime};
...
fn fixture_skill_manager() -> Arc<AcpSkillManager> {
    ...
    let paths = Arc::new(dream_core_extension::resolve_skill_paths(tmp.path(), tmp.path()));
