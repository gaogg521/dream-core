...
use dream_core_db::{ISkillRepository, SqliteSkillRepository, UpsertSkillParams, init_database_memory};
use dream_core_extension::{BUILTIN_SKILLS_ENV_VAR, resolve_skill_paths};
...
fn build_index_text_contains_load_protocol() {
    let skills = vec![
        dream_core_ai_agent::SkillIndex {
            name: "security".into(),
...
fn system_instructions_with_loaded_skills() {
    let skills = vec![dream_core_ai_agent::SkillDefinition {
        name: "helper".into(),
...
fn first_message_with_skills_index_for_acp() {
    let skills = vec![dream_core_ai_agent::SkillIndex {
        name: "review".into(),
