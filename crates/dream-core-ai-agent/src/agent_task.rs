...
pub trait IAgentTask: Send + Sync {
    ...
    /// `dream_core_session::Capabilities::supports_midturn_delivery` — defaults
...
pub trait IMockAgent: IAgentTask {
    ...
    fn answer_ask(
        &self,
        _request_id: &str,
        _answers: Option<Vec<dream_core_api_types::AskQuestionAnswer>>,
    ) -> Result<(), AgentError> {
    ...
    async fn mode(&self) -> Result<dream_core_api_types::AgentModeResponse, AgentError> {
...
impl AgentInstance {
    ...
    pub fn kill_and_wait(
        ...
        match self {
            ...
            Self::DreamEngine(m) => m.kill_and_wait(reason),
    ...
    pub fn confirm(
        ...
        match self {
            ...
            Self::DreamEngine(m) => m.confirm(msg_id, call_id, data, always_allow),
    ...
    pub fn get_session_key(&self) -> Option<String> {
        match self {
            Self::Acp(_) | Self::DreamEngine(_) | Self::Session(_) => None,
            #[cfg(any(test, feature = "test-support"))]
    ...
    pub async fn get_mode(&self) -> Result<dream_core_api_types::AgentModeResponse, AgentError> {
        match self {
            Self::Acp(m) => m.mode().await,
            Self::DreamEngine(m) => m.mode().await,
    ...
    pub async fn get_model(&self) -> Result<GetModelInfoResponse, AgentError> {
        match self {
            ...
            Self::DreamEngine(_) => Ok(GetModelInfoResponse { model_info: None }),
    ...
    pub async fn get_config_options(&self) -> Result<GetConfigOptionsResponse, AgentError> {
        match self {
            ...
            Self::DreamEngine(m) => m.config_options().await,
    ...
    pub async fn set_config_option(&self, option_id: &str, value: &str) -> Result<SetConfigOptionResponse, AgentError> {
        ...
        match self {
            ...
            Self::DreamEngine(m) => m.set_config_option(option_id, value).await,
    ...
    /// non-ACP variants (claude/codex `Session`, `DreamEngine`, `Mock`) — the
    ...
    pub async fn get_slash_commands(&self) -> Result<Vec<SlashCommandItem>, AgentError> {
        match self {
            ...
            Self::DreamEngine(m) => m.get_slash_commands().await,
    ...
    pub async fn handle_side_question(&self, req: SideQuestionRequest) -> Result<SideQuestionResponse, AgentError> {
        ...
        match self {
            ...
            Self::DreamEngine(_) => Ok(SideQuestionResponse {
                status: "unsupported".into(),
                answer: None,
            }),
...
#[cfg(test)]
mod dream_engine_config_option_tests {
    use std::sync::Arc;
...
mod required_full_auto_dispatch_tests {
    ...
    /// Non-ACP variants (proxy for claude/codex `Session`, `DreamEngine`) never
