...
pub struct SkillDefinition {
    ...
    pub source: dream_core_extension::SkillSource,
...
impl AcpSkillManager {
    pub fn new(paths: Arc<dream_core_extension::SkillPaths>) -> Arc<Self> {
        Arc::new(Self {
    ...
    /// Discover skills via `dream_core_extension::list_available_skills`.
    ///
    /// Filtering rules:
    ...
    pub async fn discover_skills_for_user(
        ...
        for item in items {
            ...
            let keep = match item.source {
                dream_core_extension::SkillSource::Builtin => {
                    if is_auto_inject {
                ...
                dream_core_extension::SkillSource::Custom
                | dream_core_extension::SkillSource::Cron
                | dream_core_extension::SkillSource::Extension => {
                    enabled_skills.is_some_and(|en| en.iter().any(|n| n == &item.name))
                }
                // Mixed distribution model: admin-required team skills load
                // without a per-assistant opt-in; optional ones stay opt-in.
                dream_core_extension::SkillSource::Team => {
    ...
    async fn list_available_skills_for_user(
        &self,
        user_id: &str,
    ) -> Result<Vec<dream_core_extension::SkillListItem>, dream_core_extension::ExtensionError> {
        if let Some(repo) = &self.skill_repo {
            dream_core_extension::list_available_skills_with_repo_for_user(&self.paths, repo.as_ref(), user_id).await
        } else {
            ...
            dream_core_extension::list_available_skills(&self.paths).await
        }
    }
    ...
    /// - `Builtin` → `dream_core_extension::read_builtin_skill(&paths, relative)`
        ...
        let content = match def.source {
            dream_core_extension::SkillSource::Builtin => {
                if let Some(rel) = def.relative_location.as_deref() {
                    match dream_core_extension::read_builtin_skill(&self.paths, rel).await {
                        Ok(s) => s,
                        Err(e) => {
...
mod tests {
    ...
    async fn new_accepts_skill_paths() {
        ...
        let paths = std::sync::Arc::new(dream_core_extension::resolve_skill_paths(tmp.path(), tmp.path()));
    ...
    fn skill_definition_has_source_and_relative_location() {
        let def = SkillDefinition {
            ...
            source: dream_core_extension::SkillSource::Builtin,
            relative_location: Some("auto-inject/x/SKILL.md".into()),
            body: None,
        };
        assert_eq!(def.source, dream_core_extension::SkillSource::Builtin);
    ...
    // because they now need `dream_core_extension::BUILTIN_SKILLS_ENV_VAR` to
    ...
    async fn get_skill_unknown_returns_none() {
        let tmp = TempDir::new().unwrap();
        let mgr = AcpSkillManager::new(std::sync::Arc::new(dream_core_extension::resolve_skill_paths(
            tmp.path(),
            tmp.path(),
        )));
