...
impl TerminalRegistry {
    ...
    pub async fn create(&self, params: CreateTerminalParams) -> Result<String, String> {
        ...
        let mut builder = if params.args.is_empty() {
            #[cfg(unix)]
            {
                let mut b = dream_core_runtime::Builder::new("/bin/sh");
                b.arg("-c").arg(&params.command);
                b
            }
            #[cfg(windows)]
            {
                let mut b = dream_core_runtime::Builder::new("cmd");
                b.arg("/C").arg(&params.command);
                b
            }
        } else {
            let mut b = dream_core_runtime::Builder::new(&params.command);
            b.args(&params.args);
            b
        };
        builder
    ...
    pub async fn kill(&self, terminal_id: &str, source: &str) -> bool {
        ...
        if let Some(c) = guard.as_mut() {
            ...
            let _ = dream_core_runtime::kill_process_tree(c).await;
            *guard = None;
