//! `SessionAgentTask` — adapts the clean-slate `dream_core_session::SessionBackend`
...
use dream_core_common::{AgentKillReason, ConversationStatus, TimestampMs, now_ms};
...
use dream_core_api_types::{AcpBuildExtra, TEAM_MCP_SERVER_NAME};
use dream_core_common::AgentType;
use dream_core_db::{IAcpSessionRepository, IMcpServerRepository, SaveRuntimeStateParams};
...
fn resolve_current_model_efforts(models: &[dream_core_session::ModelInfo], current_model: Option<&str>) -> Vec<String> {
...
/// handshake, in the SAME `dream_core_session` shape the getters read off live
...
struct CatalogPreload {
    available_models: Vec<dream_core_session::ModelInfo>,
    current_model: Option<String>,
    available_modes: Vec<dream_core_session::ModeInfo>,
...
impl CatalogPreload {
    ...
    fn from_handshake(handshake: &dream_core_api_types::AgentHandshake) -> Self {
        ...
        let (available_models, current_model) = handshake
            ...
            .map(|state| {
                let models = state
                    .available_models
                    .iter()
                    .map(|m| dream_core_session::ModelInfo {
        ...
        let (available_modes, current_mode) = handshake
            ...
            .map(|state| {
                let modes = state
                    .available_modes
                    .iter()
                    .map(|m| dream_core_session::ModeInfo {
                        id: m.id.to_string(),
                        name: m.name.clone(),
                        description: m.description.clone(),
                    })
...
impl SessionAgentTask {
    ...
    pub fn new_with_preload(
        ...
        backend: Arc<dyn SessionBackend>,
        session_repo: Option<Arc<dyn IAcpSessionRepository>>,
        handshake: &dream_core_api_types::AgentHandshake,
        prompt_dump: Option<SessionPromptDump>,
    ...
    ) -> dream_core_session::PermissionDecision {
        self.backend.request_external_permission(tool_name, input).await
    }

    pub fn get_confirmations(&self) -> Vec<dream_core_common::Confirmation> {
        self.backend
            ...
            .map(|p| {
                ...
                dream_core_common::Confirmation {
                    ...
                    options: options
                        .into_iter()
                        .map(|o| dream_core_common::ConfirmationOption {
                            label: o.name,
    ...
    pub fn answer_ask(
        ...
        let answers = answers.map(|list| {
            list.into_iter()
                .map(|a| dream_core_session::QuestionAnswer {
                    question: a.question,
                    labels: a.labels,
                })
                .collect::<Vec<_>>()
        ...
        tokio::spawn(async move {
            let command = dream_core_session::Command::AnswerAsk {
    ...
    pub fn confirm(
        ...
        use dream_core_session::PermissionDecision;
    ...
    fn effective_catalog(
        ...
        Vec<dream_core_session::ModelInfo>,
        Option<String>,
        Vec<dream_core_session::ModeInfo>,
        Option<String>,
    ) {
    ...
    pub async fn mode(&self) -> Result<dream_core_api_types::AgentModeResponse, AgentError> {
        let caps = self.backend.capabilities();
        ...
        let current_mode = caps.current_mode.or_else(|| self.catalog_preload.current_mode.clone());
        Ok(dream_core_api_types::AgentModeResponse {
    ...
    pub async fn get_model(&self) -> Result<dream_core_api_types::GetModelInfoResponse, AgentError> {
        ...
        if models.is_empty() && current_model.is_none() && override_model.is_none() {
            return Ok(dream_core_api_types::GetModelInfoResponse { model_info: None });
        }
        let available_models: Vec<dream_core_api_types::ModelInfoEntry> = models
            .iter()
            .map(|m| dream_core_api_types::ModelInfoEntry {
                id: m.id.clone(),
                label: m.name.clone(),
            })
            .collect();
        ...
        Ok(dream_core_api_types::GetModelInfoResponse {
            model_info: Some(dream_core_api_types::ModelInfoPayload {
    ...
    pub async fn get_config_options(&self) -> Result<dream_core_api_types::GetConfigOptionsResponse, AgentError> {
        // Live catalog wins; cold-start resume falls back to the persisted-handshake
        ...
        if !modes.is_empty() {
            config_options.push(dream_core_api_types::AcpConfigOptionDto {
                id: "mode".into(),
                name: Some("Mode".into()),
                label: None,
                description: None,
                ...
                options: modes
                    .iter()
                    .map(|m| dream_core_api_types::AcpConfigSelectOptionDto {
                        value: m.id.clone(),
        ...
        if !efforts.is_empty() {
            config_options.push(dream_core_api_types::AcpConfigOptionDto {
                ...
                options: efforts
                    .iter()
                    .map(|e| dream_core_api_types::AcpConfigSelectOptionDto {
                        value: e.clone(),
                        name: Some(e.clone()),
                        label: None,
                        description: None,
        ...
        Ok(dream_core_api_types::GetConfigOptionsResponse { config_options })
    }
    ...
    pub async fn set_config_option(
        ...
        option_id: &str,
        value: &str,
    ) -> Result<dream_core_api_types::SetConfigOptionResponse, AgentError> {
        ...
        let deferred = option_id == "mode"
            && self.backend.capabilities().mode_switch_effect == dream_core_session::ModeSwitchEffect::NextTurn;
        ...
        match option_id {
            ...
            _ => {
                return Ok(dream_core_api_types::SetConfigOptionResponse {
                    confirmation: dream_core_api_types::ConfigOptionConfirmation::CommandAck,
                    config_options: None,
                });
            }
        }
        ...
        let observed = snapshot
            ...
            == Some(value);
        Ok(dream_core_api_types::SetConfigOptionResponse {
            ...
            confirmation: if deferred {
                dream_core_api_types::ConfigOptionConfirmation::PendingNextTurn
            } else if observed {
                dream_core_api_types::ConfigOptionConfirmation::Observed
            } else {
                dream_core_api_types::ConfigOptionConfirmation::CommandAck
            },
    ...
    pub async fn get_slash_commands(&self) -> Result<Vec<dream_core_api_types::SlashCommandItem>, AgentError> {
        let caps = self.backend.capabilities();
        Ok(caps
            .slash_commands
            .iter()
            .map(|c| dream_core_api_types::SlashCommandItem {
    ...
    async fn cancel(&self) -> Result<(), AgentError> {
        ...
        self.backend
            .dispatch(Command::Cancel {
                target: dream_core_session::CancelTarget::Turn,
            })
            .await
            .map(|_| ())
...
pub struct SessionBuildInputs<'a> {
    ...
    pub metadata: &'a dream_core_api_types::AgentMetadata,
...
pub(crate) fn resolved_session_mode(
    ...
    session_snapshot: Option<&PersistedSessionState>,
    metadata: &dream_core_api_types::AgentMetadata,
) -> Option<String> {
    session_snapshot
...
fn spec_mode_model(
    ...
    session_snapshot: Option<&PersistedSessionState>,
    metadata: &dream_core_api_types::AgentMetadata,
) -> (dream_core_session::SessionSpec, Option<String>, Option<String>) {
    use dream_core_session::SessionSpec;
...
pub async fn build_antigravity_instance(
    ...
    spawner: Arc<dyn dream_core_process::Spawner>,
) -> Result<crate::agent_task::AgentInstance, AgentError> {
    use dream_core_session::{AntigravityConnection, BackendConnection, McpServerSpec, SessionConfig, SessionInit};
    ...
    let init = SessionInit {
        ...
        resume: matches!(spec, dream_core_session::SessionSpec::Resume { .. }),
    };
    ...
    let backend = AntigravityConnection::new(spawner)
        ...
        .await
        .map_err(|e| match e {
            dream_core_session::BackendError::WorkspaceUnavailable(path) => {
    ...
    if backend_label == "claude" {
        ...
        if !provider_env.is_empty() {
            ...
            session_config.spawn_env.extend(
                provider_env
                    .into_iter()
                    .map(|(name, value)| dream_core_common::EnvVar { name, value }),
            );
...
fn resolve_session_cli_program(
    ...
    dream_core_runtime::resolve_command_path(backend_label)
}
...
) -> Vec<dream_core_common::EnvVar> {
    let mut env: Vec<dream_core_common::EnvVar> = agent_env
        .iter()
        ...
        .collect();
    env.extend(runtime_env.iter().map(|(name, value)| dream_core_common::EnvVar {
        name: name.clone(),
        value: value.clone(),
    }));
    env
...
fn build_session_cli_config_dump_value(backend: &str, cfg: &dream_core_session::SessionConfig) -> serde_json::Value {
    use dream_core_session::McpTransport;
...
fn session_server_to_spec(server: &dream_core_api_types::SessionMcpServer) -> dream_core_session::McpServerSpec {
    use dream_core_api_types::SessionMcpTransport as T;
    use dream_core_session::{McpServerSpec, McpTransport};
...
fn team_mcp_server_spec(cfg: &dream_core_api_types::TeamMcpStdioConfig) -> dream_core_session::McpServerSpec {
    use dream_core_api_types::TeamMcpStdioConfig as C;
    dream_core_session::McpServerSpec {
        name: dream_core_api_types::TEAM_MCP_SERVER_NAME.to_owned(),
        transport: dream_core_session::McpTransport::Stdio {
...
pub fn spawn_catalog_writeback(
    agent_id: String,
    user_id: String,
    backend: Arc<dyn dream_core_session::SessionBackend>,
...
/// going empty).
fn catalog_partial_from_caps(caps: &dream_core_session::Capabilities) -> Option<dream_core_api_types::AgentHandshake> {
    let mut config_options = Vec::new();
    ...
    Some(dream_core_api_types::AgentHandshake {
        config_options,
        available_modes,
        available_models,
        available_commands,
...
fn emit_config_options_snapshot(modes: &[ModeInfo], models: &[ModelInfo], runtime: &SessionRuntime) {
    ...
    let mut config_options: Vec<dream_core_api_types::AcpConfigOptionDto> = Vec::new();
    if !modes.is_empty() {
        config_options.push(dream_core_api_types::AcpConfigOptionDto {
            id: "mode".into(),
            name: Some("Mode".into()),
...
fn spawn_event_pump(
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            {
                use dream_core_session::{SubagentStatus, SubagentTaskKind};
                match status {
            ...
            let suppress_intermediate_finish = matches!(&env.event, SessionEvent::TurnResult { is_error, outcome, .. }
                if !workflow_inflight.is_empty()
                    && !*is_error
                    && !matches!(outcome, dream_core_session::TurnOutcome::Cancelled { .. }));
                ...
                SessionEvent::TurnResult { .. } | SessionEvent::Detached { .. } if !suppress_intermediate_finish => {
                    ...
                    let keep_background = matches!(
                        ...
                        SessionEvent::TurnResult { is_error: false, outcome, .. }
                            if !matches!(outcome, dream_core_session::TurnOutcome::Cancelled { .. })
                    );
                    for data in settle_workflow_cards(
                        &mut workflow_cards,
                        crate::workflow_progress::CardStatus::Cancelled,
...
/// `Orchestrator` uses (`dream_core_session::is_unrecoverable_resume_error`), so a
...
fn is_dead_resume_anchor(event: &SessionEvent) -> bool {
    use dream_core_session::{TurnOutcome, is_unrecoverable_resume_error};
    ...
    let reason = dream_core_session::ErrorReason::Backend {
...
fn broadcast_usage_frame(bus: &dyn EventBroadcaster, conversation_id: &str, user_id: &str, event: &SessionEvent) {
    ...
    dream_core_common::normalize_keys_to_snake_case(&mut event_data);
    ...
    bus.broadcast(dream_core_api_types::WebSocketMessage::new("message.stream", payload));
}
...
fn empty_turn_tip(outcome: &dream_core_session::TurnOutcome) -> Option<TipsEventData> {
    use dream_core_session::{StopReason, TruncationKind, TurnOutcome};
...
fn translate_event(event: SessionEvent, conversation_id: &str, terminal_result_seen: bool) -> Vec<AgentStreamEvent> {
    match event {
        ...
        SessionEvent::TurnResult {
            ...
            let is_cancel = matches!(outcome, dream_core_session::TurnOutcome::Cancelled { .. });
        ...
        SessionEvent::Detached { exit, redacted_summary } => {
            ...
            match dream_core_session::crash_outcome(terminal_result_seen, exit) {
                dream_core_session::Outcome::Crashed => {
                ...
                dream_core_session::Outcome::CleanNoResult | dream_core_session::Outcome::FollowResult => {
                    vec![AgentStreamEvent::Finish(FinishEventData::default())]
                }
            }
        }
            ...
            let entries: Vec<serde_json::Value> = entries
                ...
                .map(|e| {
                    let status = match e.status {
                        dream_core_session::PlanStatus::Pending => "pending",
                        dream_core_session::PlanStatus::InProgress => "in_progress",
                        dream_core_session::PlanStatus::Completed => "completed",
                    };
        ...
        SessionEvent::Notice {
            ...
            let tip_type = match level {
                dream_core_session::NoticeLevel::Info => TipType::Info,
                dream_core_session::NoticeLevel::Warning => TipType::Warning,
            };
...
mod build_mapping_tests {
    ...
    fn extra_with_thought_level(level: Option<&str>) -> dream_core_api_types::AcpBuildExtra {
        dream_core_api_types::AcpBuildExtra {
    ...
    fn assemble_spawn_env_orders_agent_overrides_before_runtime_context() {
        let agent_env = vec![
            dream_core_api_types::AgentEnvEntry {
    ...
    fn session_cli_config_dump_value_captures_resolved_surface() {
        use dream_core_session::{McpServerSpec, McpTransport, SessionConfig, SessionInit};
    ...
    fn test_metadata(backend: Option<&str>, yolo_id: Option<&str>) -> dream_core_api_types::AgentMetadata {
        use dream_core_api_types::{AgentHandshake, AgentMetadata, AgentSource, AgentSourceInfo, BehaviorPolicy};
        use dream_core_common::AgentType;
        AgentMetadata {
            id: "test".into(),
            icon: None,
    ...
    struct DirectMcpRepo {
        rows: Vec<dream_core_db::models::McpServerRow>,
    }

    #[derive(Default)]
    struct RecordingFailSpawner {
        last_command: std::sync::Mutex<Option<dream_core_common::CommandSpec>>,
    }

    impl RecordingFailSpawner {
        fn last_command(&self) -> Option<dream_core_common::CommandSpec> {
            self.last_command.lock().unwrap().clone()
        }
    }

    #[async_trait::async_trait]
    impl dream_core_process::Spawner for RecordingFailSpawner {
        async fn spawn(
            &self,
            spec: dream_core_common::CommandSpec,
            _extra_env: &[(String, String)],
            _opaque_owner_tag: &str,
        ) -> Result<Arc<dream_core_process::ManagedProcess>, dream_core_process::ProcessError> {
            *self.last_command.lock().unwrap() = Some(spec);
            Err(dream_core_process::ProcessError::internal(
    ...
    impl IMcpServerRepository for DirectMcpRepo {
        ...
        async fn find_by_id(
            ...
            user_id: &str,
            id: &str,
        ) -> Result<Option<dream_core_db::models::McpServerRow>, dream_core_db::DbError> {
            Ok(self
                .rows
                .iter()
        ...
        ) -> Result<dream_core_db::models::McpServerRow, dream_core_db::DbError> {
    ...
    fn direct_mcp_row(id: &str, name: &str) -> dream_core_db::models::McpServerRow {
        dream_core_db::models::McpServerRow {
            id: id.into(),
            user_id: "user-1".into(),
            name: name.into(),
            description: None,
    ...
    async fn direct_claude_spawn_contains_team_nonbuiltin_and_builtin_without_reserved_override() {
        use dream_core_api_types::{SessionMcpServer, SessionMcpTransport, TeamMcpStdioConfig};
        ...
        let broadcaster: Arc<dyn EventBroadcaster> = Arc::new(dream_core_realtime::BroadcastEventBus::new(16));
    ...
    fn metadata_with_models(ids: &[&str]) -> dream_core_api_types::AgentMetadata {
    ...
    fn spec_fork_quadrants() {
        let fork_cfg = AcpBuildExtra {
            fork: Some(dream_core_api_types::ForkSpec {
    ...
    fn catalog_partial_projects_discovered_modes_models_commands() {
        use dream_core_session::{ModeInfo, ModelInfo, SlashCommandInfo};
        let caps = dream_core_session::Capabilities {
        ...
        let empty = dream_core_session::Capabilities::default();
        assert!(
...
mod translate_tests {
    use super::*;
    use crate::protocol::events::tool_call::{ToolCallEventData, ToolCallStatus};
    use dream_core_session::PermissionKind;
    ...
    fn breakdown_rides_meta_under_the_renderer_key_names() {
        let with = translate_event(
            SessionEvent::UsageDelta {
                ...
                breakdown: dream_core_session::UsageBreakdown {
    ...
    fn plan_surfaces_with_snake_case_status() {
        use dream_core_session::{PlanEntry, PlanStatus};
        let events = translate_event(
            SessionEvent::Plan {
                entries: vec![
                    PlanEntry {
    ...
    fn cancelled_turn_emits_finish_without_error() {
        use dream_core_session::{CancelReason, TurnOutcome};
        let events = translate_event(
            SessionEvent::TurnResult {
                is_error: true,
                api_error_status: None,
    ...
    fn errored_turn_emits_rich_error_terminal() {
        use dream_core_session::{StopReason, TurnOutcome};
        let events = translate_event(
            SessionEvent::TurnResult {
                is_error: true,
    ...
    fn detached_crash_surfaces_as_rich_disconnect_error() {
        use dream_core_session::ExitStatusLite;
        let events = translate_event(
    ...
    fn tip_code(outcome: dream_core_session::TurnOutcome) -> Option<(TipType, String)> {
...
mod persist_tests {
    ...
    async fn seeded_repo() -> (Arc<dyn IAcpSessionRepository>, dream_core_db::Database) {
    ...
    async fn usage_arriving_after_the_turn_ended_is_still_persisted() {
        ...
        persist_side_effects(
            ...
            &SessionEvent::TurnResult {
                ...
                epoch: 0,
                outcome: dream_core_session::TurnOutcome::EndTurn,
            },
        )
        .await;
    ...
    fn broadcast_frame_matches_the_renderer_contract() {
        #[derive(Default)]
        struct Recorder(std::sync::Mutex<Vec<dream_core_api_types::WebSocketMessage<serde_json::Value>>>);
        impl EventBroadcaster for Recorder {
            fn broadcast(&self, event: dream_core_api_types::WebSocketMessage<serde_json::Value>) {
    ...
    async fn send_dispatch_session_not_found_clears_anchor_and_classifies() {
        ...
        assert_eq!(
            err.code(),
            Some(dream_core_api_types::AgentErrorCode::UserAgentSessionNotFound),
    ...
    impl SessionBackend for EffortCapsBackend {
        ...
        fn capabilities(&self) -> Capabilities {
            use dream_core_session::ModelInfo;
            Capabilities {
    ...
    fn is_dead_resume_anchor_matrix() {
        use dream_core_session::{CancelReason, StopReason, TurnOutcome};
...
mod pump_tests {
    ...
    struct RecordingBackend {
        ...
        blocks: dream_core_session::BlockSet,
    }
    ...
    async fn send_message_partitions_image_into_native_block() {
        let dir = std::env::temp_dir().join("dream-core-session-media-tests");
    ...
    async fn send_message_pairs_image_block_with_resource_link() {
        let dir = std::env::temp_dir().join("dream-core-session-media-link-tests");
        std::fs::create_dir_all(&dir).unwrap();
    ...
    async fn clean_turn_end_keeps_detached_exec_card_but_cancels_others() {
        use dream_core_session::TurnOutcome;
        ...
        let clean_end = SessionEvent::TurnResult {
            ...
            epoch: 0,
            outcome: TurnOutcome::Completed {
                stop_reason: dream_core_session::StopReason::EndTurn,
            },
        };
    ...
    async fn cancelled_turn_closes_open_tool_calls_as_canceled_before_finish() {
        use dream_core_session::{CancelReason, SubagentKind, TurnOutcome};
        let script = vec![
    ...
    async fn completed_tool_call_is_not_recanceled_at_turn_end() {
        use dream_core_session::SubagentKind;
        let script = vec![
            env(SessionEvent::TurnStarted { epoch: 1 }),
            env(SessionEvent::ToolCall {
                tool_use_id: "call-1".into(),
    ...
    async fn catalog_updated_projects_config_option_with_model_and_mode() {
        use dream_core_session::{ModeInfo, ModelInfo};
    ...
    async fn catalog_updated_projects_available_commands_frame() {
        use dream_core_session::SlashCommandInfo;
        let script = vec![env(SessionEvent::CatalogUpdated {
            models: Vec::new(),
            modes: Vec::new(),
            slash_commands: vec![
    ...
    async fn workflow_launch_result_finish_is_suppressed_until_workflow_completes() {
        use dream_core_session::SubagentStatus;
        let script = vec![
            env(SessionEvent::ToolCall {
                tool_use_id: "toolu_wf".into(),
                name: "Task".into(),
                subagent: dream_core_session::SubagentKind::Workflow,
                input: serde_json::Value::Null,
                parent_tool_use_id: None,
            }),
            ...
            env(SessionEvent::SubagentUpdate {
                ...
                kind: Some(dream_core_session::SubagentTaskKind::WorkflowContainer),
            }),
            env(SessionEvent::MessageDelta {
                item_id: "m".into(),
    ...
    fn wf_container(
        status: dream_core_session::SubagentStatus,
        kind: Option<dream_core_session::SubagentTaskKind>,
    ) -> SessionEvent {
        SessionEvent::SubagentUpdate {
            r#ref: "task-wf".into(),
            label: Some("wf".into()),
    ...
    async fn workflow_progress_streams_while_the_workflow_runs() {
        use dream_core_session::{SubagentStatus, SubagentTaskKind, WorkflowLoopState};
        let script = vec![
            env(SessionEvent::ToolCall {
                tool_use_id: "toolu_wf".into(),
                name: "Workflow".into(),
    ...
    async fn background_bash_gets_a_live_card_and_settles_on_notification() {
        ...
        let script = vec![
            ...
            env(SessionEvent::ToolResult {
                tool_use_id: "toolu_bg".into(),
                is_error: false,
                content: vec![dream_core_session::ToolResultContent::Text(
    ...
    async fn cancelled_turn_settles_background_cards() {
        use dream_core_session::{CancelReason, SubagentStatus, SubagentTaskKind};
        let script = vec![
            env(SessionEvent::ToolCall {
                tool_use_id: "toolu_bg".into(),
                name: "Bash".into(),
            ...
            env(SessionEvent::TurnResult {
                ...
                result_text: String::new(),
                epoch: 0,
                outcome: dream_core_session::TurnOutcome::Cancelled {
                    reason: CancelReason::UserCancel,
                },
            }),
        ];
    ...
    struct PendingPermBackend(dream_core_session::PendingPermissionView);
    ...
    impl SessionBackend for PendingPermBackend {
        ...
        fn pending_permission_requests(&self) -> Vec<dream_core_session::PendingPermissionView> {
            vec![self.0.clone()]
        }
    }
    ...
    impl SessionBackend for NextTurnEffectBackend {
        ...
        fn capabilities(&self) -> Capabilities {
            Capabilities {
                mode_switch_effect: dream_core_session::ModeSwitchEffect::NextTurn,
                ..StaticCapsBackend.capabilities()
            }
        }
    }
    ...
    fn handshake_with_catalog() -> dream_core_api_types::AgentHandshake {
        dream_core_api_types::AgentHandshake {
    ...
    async fn live_capabilities_overwrite_stale_preload() {
        ...
        let stale = dream_core_api_types::AgentHandshake {
            available_models: Some(serde_json::json!({
...
mod force_kill_tests {
    ...
    use dream_core_common::{AgentKillReason, ConversationStatus};
    use dream_core_session::{
        Admission, BackendError, Capabilities, Command, CommandReceipt, SessionBackend, SessionEnvelope,
    };
    use futures_util::stream::BoxStream;
...
mod cold_start_effort_tests {
    ...
    fn handshake_with(models: serde_json::Value) -> dream_core_api_types::AgentHandshake {
        dream_core_api_types::AgentHandshake {
...
mod catalog_writeback_tests {
    ...
    use super::*;
    use dream_core_session::{Admission, Capabilities, CommandReceipt, ModeInfo, ModelInfo};
    use std::sync::atomic::{AtomicUsize, Ordering};
