...
impl AgentRegistry {
    ...
    pub async fn run_slow_probe_recheck(&self) {
        ...
        for id in pending {
            ...
            let params = dream_core_db::UpdateAgentAvailabilitySnapshotParams {
...
fn decode_row(
    ...
    let runtime_backend = dream_core_db::runtime_backend_for_agent(&row);
    ...
    let descriptor_team_capable = dream_core_session::backend_capability_descriptor(&runtime_backend)
        .map(dream_core_session::BackendCapabilityDescriptor::resolved)
    ...
    let team_capable = behavior_policy.supports_team
        ...
        || dream_core_common::constants::is_team_capable(persisted_backend, handshake.agent_capabilities.as_ref());
...
mod tests {
    ...
    async fn direct_cli_metadata_projects_constructed_mcp_capabilities() {
        let reg = registry().await;
        for descriptor in dream_core_session::backend_capability_descriptors()
            .iter()
            .filter(|descriptor| descriptor.origin == dream_core_common::CapabilityOrigin::DirectDescriptor)
        {
    ...
    async fn list_by_agent_type_counts_seed_rows() {
        ...
        assert_eq!(count(AgentType::DreamEngine), 1);
    }
    ...
    fn decode_row_applies_command_override() {
        use dream_core_db::AgentMetadataRow;
    ...
    fn decode_row_ignores_internal_aion_cli_overrides() {
        ...
        assert_eq!(meta.agent_type, AgentType::DreamEngine);
