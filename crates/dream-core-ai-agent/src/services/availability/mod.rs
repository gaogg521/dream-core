...
use dream_core_db::{IProviderRepository, UpdateAgentAvailabilitySnapshotParams};
            ...
            Ok(success) => {
                ...
                let drift = direct_cli_program(meta).and_then(|cli| {
                    success
                        .reported_version
                        .as_deref()
                        .and_then(|reported| dream_core_session::version_drift(cli, reported))
                });
                match drift {
...
fn explicit_probe_args(meta: &AgentMetadata) -> Result<Vec<String>, String> {
    if meta.agent_source == AgentSource::Builtin && meta.agent_source_info.bridge_binary.as_deref() == Some("npx") {
        ...
        return dream_core_runtime::pin_registry_npx_args(backend, &meta.args).map_err(|error| error.to_string());
    }
    Ok(meta.args.clone())
}
...
mod tests {
    ...
    async fn manual_check_restores_previously_offline_agent() {
        ...
        repo.update_availability_snapshot(
            ...
            &dream_core_db::UpdateAgentAvailabilitySnapshotParams {
