...
impl AgentService {
    ...
    pub async fn set_agent_overrides(
        ...
        user_id: &str,
        id: &str,
        req: dream_core_api_types::SetAgentOverridesRequest,
    ...
    pub async fn get_agent_overrides(
        ...
        user_id: &str,
        id: &str,
    ) -> Result<dream_core_api_types::AgentOverridesResponse, AgentError> {
        let row = self
            .registry
            .repo_handle()
            .get_for_user(user_id, id)
        ...
        let env_override = row
            .env_override
            .as_deref()
            .and_then(|s| serde_json::from_str::<Vec<dream_core_api_types::AgentEnvEntry>>(s).ok())
            .unwrap_or_default();
...
fn is_bridge_launched_row(row: &dream_core_db::AgentMetadataRow) -> bool {
    let Some(raw) = row.agent_source_info.as_deref() else {
        return false;
    };
    let Ok(info) = serde_json::from_str::<dream_core_api_types::AgentSourceInfo>(raw) else {
        return false;
    };
