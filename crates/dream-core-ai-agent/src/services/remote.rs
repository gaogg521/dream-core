...
use dream_core_common::{RemoteAgentAuthType, RemoteAgentProtocol, RemoteAgentStatus, decrypt_string, encrypt_string};
use dream_core_db::models::RemoteAgentRow;
use dream_core_db::{IRemoteAgentRepository, UpdateRemoteAgentParams};
use base64::Engine;
...
impl RemoteAgentService {
    ...
    pub async fn create(
        ...
        let row = self
            .repo
            .create(dream_core_db::CreateRemoteAgentParams {
                user_id,
                name: &req.name,
    ...
    pub async fn update(
        ...
        let row = self.repo.update(user_id, id, params).await.map_err(|e| match e {
            dream_core_db::DbError::NotFound(msg) => AgentError::not_found(msg),
...
fn generate_device_keypair(encryption_key: &[u8; 32]) -> Result<(String, String, String), AgentError> {
    ...
    let device_id = dream_core_common::generate_prefixed_id("dev");
...
fn db_err(e: dream_core_db::DbError) -> AgentError {
