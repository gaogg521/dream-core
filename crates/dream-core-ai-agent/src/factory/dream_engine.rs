...
pub(super) async fn build(
    ...
    let api_key = dream_core_common::decrypt_string(&row.api_key_encrypted, &deps.encryption_key)
...
pub(crate) fn resolve_bedrock_config(json: Option<&str>) -> Option<dream_engine_config::config::BedrockConfig> {
    let bc: dream_core_api_types::BedrockConfig = serde_json::from_str(json?).ok()?;
    Some(dream_engine_config::config::BedrockConfig {
...
pub async fn resolve_provider_config_for_bridge(
    ...
    let cli_args = dream_engine_config::config::CliArgs {
...
mod tests {
    ...
    impl IMcpServerRepository for MockMcpRepo {
        ...
        async fn find_by_id(&self, user_id: &str, id: &str) -> Result<Option<McpServerRow>, dream_core_db::DbError> {
            Ok(self
                .rows
                .iter()
