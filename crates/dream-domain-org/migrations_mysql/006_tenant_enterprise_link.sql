-- Link a project-group tenant to a company (真实企业 / one-enterprise) (MySQL port).
--
...
-- (one-enterprise) and this column is resolved at the app layer; cross-crate
-- FKs would couple their apply order (same reasoning as the SQLite original).
ALTER TABLE one_tenants ADD COLUMN enterprise_id VARCHAR(255) NULL;

CREATE INDEX idx_one_tenants_enterprise ON one_tenants (enterprise_id);
