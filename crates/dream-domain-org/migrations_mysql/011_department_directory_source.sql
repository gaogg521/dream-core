...
-- department an admin typed in (MySQL port).
--
...
-- case at all — MySQL's plain unique index matches: multiple NULLs admitted.
ALTER TABLE one_departments ADD COLUMN source VARCHAR(16) NULL;
ALTER TABLE one_departments ADD COLUMN directory_external_id VARCHAR(255) NULL;

CREATE UNIQUE INDEX idx_one_departments_directory_external
