...
display_name: "YouTube 视频内容深度调研"
description: Enhance any deep research task with first-hand YouTube video evidence。触发：用户提出「YouTube 视频内容深度调研」相关需求时使用（常见说法：YouTube 视频内容深度调研、YouTube 视频内容深度调研；英文：youtube/video/research）；不要用于“生成或编辑 Word/Excel/PPT 文档文件”（改用 officecli-docx / officecli-xlsx / officecli-pptx），也不要用于与本技能无关的其他任务。需要用户提供：明确的任务描述，以及必要的输入文件或数据。
---
