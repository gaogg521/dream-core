...
display_name: "iOS 应用开发"
description: >-
  触发：用户提出「ios-application-dev」相关需求时使用（常见说法：ios-application-dev、ios-application-dev；英文：ios/application/dev）；不要用于“生成或编辑 Word/Excel/PPT 文档文件”（改用 officecli-docx / officecli-xlsx / officecli-pptx），也不要用于与本技能无关的其他任务。需要用户提供：明确的任务描述，以及必要的输入文件或数据。
---
