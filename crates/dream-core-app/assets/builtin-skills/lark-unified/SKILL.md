...
name: lark-unified
display_name: "飞书套件"
description: Unified Lark/Feishu CLI suite covering messaging, documents, spreadsheets, base tables, ca。触发：用户提出「lark-unified」相关需求时使用（常见说法：lark-unified、lark-unified；英文：lark/unified）；不要用于“生成或编辑 Word/Excel/PPT 文档文件”（改用 officecli-docx / officecli-xlsx / officecli-pptx），也不要用于与本技能无关的其他任务。需要用户提供：明确的任务描述，以及必要的输入文件或数据。
---
...
STATUS=$(find ~/.dream/skills ./.dream/skills -name meegle_status.py 2>/dev/null | head -1)
...
```bash
SETUP=$(find ~/.dream/skills ./.dream/skills -name meegle_setup.py 2>/dev/null | head -1)
...
STATUS=$(find ~/.dream/skills ./.dream/skills -name lark_status.py 2>/dev/null | head -1)
...
```bash
SETUP=$(find ~/.dream/skills ./.dream/skills -name lark_setup.py 2>/dev/null | head -1)
