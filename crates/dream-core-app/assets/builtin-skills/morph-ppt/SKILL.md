...
Verify with `officecli --version` (open a new terminal if PATH hasn't picked up). If install fails, download a binary from https://officecli.ai.
...
- "While the build script is running, the `.pptx` may be rewritten several times. If you want to preview progress, use `officecli watch "$FILE"` and open the live preview in 1One Work — do NOT click 'Open with system app' during the build, or you'll hit a file lock."
