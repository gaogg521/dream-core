...
display_name: "钉钉套件"
description: 钉钉 CLI 全能套件，基于官方 DingTalk Workspace CLI（dws）操作钉钉消息、群聊、通讯录、日历、待办、审批、考勤、日志、DING、AI 表格、钉钉文档、钉。触发：用户提出「钉钉套件」相关需求时使用（常见说法：钉钉套件、钉钉套件；英文：dingtalk/unified）；不要用于“生成或编辑 Word/Excel/PPT 文档文件”（改用 officecli-docx / officecli-xlsx / officecli-pptx），也不要用于与本技能无关的其他任务。需要用户提供：明确的任务描述，以及必要的输入文件或数据。
---
...
本 Skill 不依赖 本平台 Runtime 改造即可完成授权。按以下顺序执行：
...
- [scripts/](./scripts/)：官方批量工作流脚本和 本平台 setup 脚本
