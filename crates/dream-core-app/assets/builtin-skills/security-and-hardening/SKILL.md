...
display_name: "安全加固"
description: OWASP prevention, auth, secrets management and dependency auditing.。触发：用户提出「安全加固」相关需求时使用（常见说法：安全加固、安全加固；英文：security/and/hardening）；不要用于“生成或编辑 Word/Excel/PPT 文档文件”（改用 officecli-docx / officecli-xlsx / officecli-pptx），也不要用于与本技能无关的其他任务。需要用户提供：明确的任务描述，以及必要的输入文件或数据。
---
