...
display_name: "Android 原生开发"
description: Android native application development and UI design guide. Covers Material Design 3, Kotl。触发：用户提出「android-native-dev」相关需求时使用（常见说法：android-native-dev、android-native-dev；英文：android/native/dev）；不要用于“生成或编辑 Word/Excel/PPT 文档文件”（改用 officecli-docx / officecli-xlsx / officecli-pptx），也不要用于与本技能无关的其他任务。需要用户提供：明确的任务描述，以及必要的输入文件或数据。
---
