...
name: qq-email
display_name: "QQ邮箱"
description: QQ邮箱 IMAP receive and SMTP send via Node.js scripts; credentials read from env vars QQ_EMA。触发：用户提出「qq-email」相关需求时使用（常见说法：qq-email、qq-email；英文：qq/email）；不要用于“生成或编辑 Word/Excel/PPT 文档文件”（改用 officecli-docx / officecli-xlsx / officecli-pptx），也不要用于与本技能无关的其他任务。需要用户提供：明确的任务描述，以及必要的输入文件或数据。
---
