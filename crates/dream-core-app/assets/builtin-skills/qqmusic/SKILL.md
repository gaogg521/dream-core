...
name: qqmusic
display_name: "QQ音乐助手"
description: QQ Music — search songs, albums, playlists, music videos, artists; daily recommendations; 。触发：用户提出「QQ音乐」相关需求时使用（常见说法：QQ音乐、QQ音乐助手；英文：qqmusic）；不要用于“生成或编辑 Word/Excel/PPT 文档文件”（改用 officecli-docx / officecli-xlsx / officecli-pptx），也不要用于与本技能无关的其他任务。需要用户提供：明确的任务描述，以及必要的输入文件或数据。
---
