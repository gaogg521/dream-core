...
display_name: "微信小程序开发框架"
description: WeChat Mini Program (微信小程序) development framework. Use when building WeChat mini apps with。触发：用户提出「wechat-miniprogram」相关需求时使用（常见说法：wechat-miniprogram、wechat-miniprogram；英文：wechat/miniprogram）；不要用于“生成或编辑 Word/Excel/PPT 文档文件”（改用 officecli-docx / officecli-xlsx / officecli-pptx），也不要用于与本技能无关的其他任务。需要用户提供：明确的任务描述，以及必要的输入文件或数据。
---
