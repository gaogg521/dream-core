...
- The registration process only copies the above local directories into the local 本平台 "My Experts" marketplace (`~/.dream/plugins/marketplaces/my-experts/`) and writes a local `本平台` registration entry. **It never accesses any external download address and never executes any remote script.**
...
- **Auto-registration on first call**: When the skill is first invoked, the client **automatically registers** with the cloud MCP service (`https://mcp.aitaxs.top`) and receives a **randomly generated API key**.
- **Local storage**: The API key is stored **plaintext** in the local config file (`~/.dream/skills/<slug>/config/.skill_config.json` or equivalent). **No encryption is applied**. The key is only used for authentication with the operator's own service.
