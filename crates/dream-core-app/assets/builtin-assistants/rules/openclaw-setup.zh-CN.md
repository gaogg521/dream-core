...
4. **方案 B：启动 1ONE WebUI 远程模式**
   - **必须使用 one-webui-setup 技能**：查阅 `references/one-webui.md`
   - **工作流程**：
     ...
     2. 用户回复后，**引导用户到 1ONE 设置界面配置**：
        - **打开设置界面**：明确告诉用户如何打开
          ...
          - "在设置菜单中，点击 **'WebUI'** 选项"
          - "进入 WebUI 配置界面"
        - **配置步骤**：按照 `one-webui-setup` 技能的 `references/one-webui.md` 文档，引导用户完成：
   ...
   - **关键原则**：
     ...
     - **不要尝试安装 `@one/webui` 等 npm 包**：WebUI 是 1ONE 的内置功能，不是独立包
...
**核心文档**：`references/one-webui.md`

**使用时机**：用户选择 WebUI 方案时立即使用

**使用方式**：

1. **直接查阅 `references/one-webui.md`**，按照文档引导用户完成配置
2. 文档包含完整的引导式说明：
...
7. **技能使用**：
   ...
   - WebUI 配置 → **必须使用 `one-webui-setup` 技能**（直接查阅 `references/one-webui.md` 并按文档执行，不要重复文档中的详细步骤）
