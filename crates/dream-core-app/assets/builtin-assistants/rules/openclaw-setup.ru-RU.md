...
4. **Вариант B: Запуск 1ONE WebUI в удалённом режиме**
   - **Обязательно использовать навык one-webui-setup**: Обратиться к `references/one-webui.md`
   - **Рабочий процесс**:
     ...
     2. После ответа пользователя, **направить пользователя к интерфейсу настроек 1ONE**:
        ...
        - **Шаги настройки**: Следуйте документации `one-webui-setup` навыка `references/one-webui.md` для направления пользователя:
   ...
   - **Ключевые принципы**:
     ...
     - **Не пытайтесь установить `@one/webui` или подобные npm-пакеты**: WebUI — это встроенная функция 1ONE, а не отдельный пакет
...
### Навык one-webui-setup

**Основная документация**: `references/one-webui.md`

**Когда использовать**: Когда пользователь выбирает вариант WebUI, использовать немедленно

**Как использовать**:

1. **Напрямую обратиться к `references/one-webui.md`** и направить пользователя на завершение конфигурации согласно документации
...
3. **Выбрал WebUI**: Использовать навык `one-webui-setup` → Спросить потребности → Выбрать решение → Выполнить настройку → Предоставить инструкции по использованию
...
7. **Использование навыков**:
   ...
   - Настройка WebUI → **Обязательно использовать навык `one-webui-setup`** (напрямую обратиться к `references/one-webui.md` и следовать документации, не повторять подробные шаги из документации)
