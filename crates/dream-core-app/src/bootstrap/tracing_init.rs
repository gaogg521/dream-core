...
const NOISE_SUPPRESSIONS: &[&str] = &[
    ...
    // out of default dev logs; dream_core_ai_agent::protocol::acp emits sanitized
    // summaries for the ACP flow we need to debug.
    "agent_client_protocol::jsonrpc=info",
    // DreamEngine provider/agent debug logs include raw request bodies and SSE
    // chunks. Keep lifecycle info logs, but do not write prompt/output
    // payloads by default.
    "dream_engine_agent=info",
...
fn build_backend_filter(log_level: Option<&str>) -> EnvFilter {
    let user_directives = log_level.unwrap_or("info");
    let suppressions = NOISE_SUPPRESSIONS.join(",");
    let aionrs_off: String = ENGINE_TARGETS
        .iter()
        .map(|t| format!("{t}=off"))
        .collect::<Vec<_>>()
        .join(",");
...
mod tests {
    ...
    fn env_filter_suppresses_raw_engine_provider_debug_even_when_debug_enabled() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            assert!(
                !tracing::enabled!(target: "dream_engine_agent", Level::DEBUG),
                "dream_engine_agent debug logs include raw request bodies"
            );
            assert!(
                !tracing::enabled!(target: "dream_engine_providers", Level::DEBUG),
                "dream_engine_providers debug logs include raw SSE chunks"
            );
            assert!(
                tracing::enabled!(target: "dream_core_ai_agent::manager::dream_engine::agent", Level::DEBUG),
                "DreamEngine lifecycle debug logs should still be available"
            );
        });
    }
