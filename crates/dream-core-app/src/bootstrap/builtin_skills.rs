...
pub(super) async fn materialize_builtin_skills(data_dir: &Path) -> Result<()> {
    ...
    let corpus = dream_core_extension::builtin_skills_corpus();
    let marker = dream_core_extension::builtin_skills_materialize_marker(corpus, env!("CARGO_PKG_VERSION"));

    dream_core_extension::materialize_if_needed(data_dir, corpus, &marker)
        .await
...
mod tests {
    ...
    fn materialize_marker_includes_builtin_skill_corpus_fingerprint() {
        let marker = dream_core_extension::builtin_skills_materialize_marker(
