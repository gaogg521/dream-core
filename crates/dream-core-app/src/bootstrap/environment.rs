...
pub async fn init_data_layer(config: &AppConfig) -> Result<Database, BootstrapError> {
    ...
    dream_core_db::maybe_copy_legacy_database(&db_path).map_err(|e| {
        BootstrapError::new(
            BootstrapErrorCode::DataInitFailed,
            "data.legacy_db",
            "failed to initialize application data",
...
fn database_init_bootstrap_error(error: dream_core_db::DatabaseInitError, db_path: &std::path::Path) -> BootstrapError {
    ...
    if let Some(db_version) = source.missing_migration_version() {
        ...
        if let Some(app_version) = dream_core_db::latest_known_migration_version() {
...
mod tests {
    ...
    fn database_stage_comes_from_db_boundary_error() {
        let err = dream_core_db::DatabaseInitError::new(
            "database.migration",
            dream_core_db::DbError::Migration(sqlx::migrate::MigrateError::VersionMismatch(42)),
        );
    ...
    fn database_schema_repair_stage_comes_from_db_boundary_error() {
        let err = dream_core_db::DatabaseInitError::new(
            ...
            dream_core_db::DbError::Init("repair failed".into()),
        );
    ...
    fn database_recoverable_corruption_stage_comes_from_db_boundary_error() {
        let err = dream_core_db::DatabaseInitError::new(
            ...
            dream_core_db::DbError::Migration(sqlx::migrate::MigrateError::ExecuteMigration(
    ...
    fn newer_than_app_stage_carries_migration_versions_on_stderr() {
        let err = dream_core_db::DatabaseInitError::new(
            dream_core_db::DATABASE_NEWER_THAN_APP_STAGE,
        ...
        let app_version = dream_core_db::latest_known_migration_version().unwrap();
