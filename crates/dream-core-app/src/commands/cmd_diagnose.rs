...
const ENV_LOG_DIR: &str = "ONE_LOG_DIR";
    ...
    let log_dir = optional_string_field(&payload, "log_dir")
        ...
        .ok_or_else(|| {
            DiagnoseError::new(
                ...
                command,
                "missing log directory; set ONE_LOG_DIR or pass log_dir in stdin JSON",
            )
...
/// Suffixes written by the backend's file layers today, plus the pre-rebrand
/// ones so an install that already has `*.aioncore.log` / `*.aionrs.log`
/// history stays diagnosable. Kept in sync with dream-ui's `LOG_SUFFIXES` in
/// `packages/desktop/src/process/feedback/logs.ts`.
///
...
mod tests {
    ...
    fn diagnose_error_renders_quoted_stable_contract() {
        ...
        assert_eq!(
            error.stderr_line(),
            "DIAGNOSE_ENV_MISSING command=\"diagnose context\" field=\"ONE_CONVERSATION_ID\": missing required environment variable"
        );
    }
    ...
    #[test]
    fn collect_backend_logs_finds_current_and_legacy_suffixes() {
        ...
        let mut files = Vec::new();
        collect_backend_logs(dir.path(), 0, &mut files).unwrap();
