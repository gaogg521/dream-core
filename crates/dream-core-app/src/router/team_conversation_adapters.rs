...
use dream_core_db::models::{AgentMetadataRow, MessageRow};
use dream_core_db::{IAgentMetadataRepository, IConversationRepository};
use dream_core_team::{
...
impl NativeSlashCommandPort for TeamConversationAdapters {
    ...
    /// the SHARED grammar owner (`dream_core_session::slash_command_name`, same
    ...
    async fn recognize(&self, conversation_id: &str, content: &str) -> SlashCommandRecognition {
        let Some(name) = dream_core_session::slash_command_name(content) else {
...
impl TeamConversationAdapters {
    ...
    async fn resolve_slash_catalog(&self, conversation_id: &str) -> Option<(Vec<String>, SlashCatalogSource)> {
        ...
        {
            let names = dream_core_session::codex_capabilities()
                .slash_commands
                .into_iter()
