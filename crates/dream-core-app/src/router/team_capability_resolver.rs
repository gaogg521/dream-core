...
use dream_core_common::{CapabilityOrigin, McpTransportCapabilities, ResolvedBackendCapabilities};
use dream_core_db::{IAgentMetadataRepository, resolve_agent_binding_from_rows};
use dream_core_team::{TeamError, TeamToolCapabilityPort};
...
impl TeamToolCapabilityPort for TeamCapabilityResolver {
    async fn resolve(
        ...
        if let Some(descriptor) = dream_core_session::backend_capability_descriptor(backend) {
