...
use cli::{Cli, Command};
use dream_core_app::AppServices;
...
fn run_main() -> Result<ExitCode, MainError> {
    ...
    // parse a uniform CLI, but bypass `dream_core_runtime::init` (which would
    ...
    // detection path exactly. It must hit the same `dream_core_runtime::init`
    // before performing managed-runtime and PATH probing.
    let needs_runtime = cli.command.as_ref().is_none_or(Command::need_runtime);
    if needs_runtime {
        dream_core_runtime::set_managed_resources_mode(cli.managed_resources_mode.into());
    ...
    let merged_path = unsafe { dream_core_runtime::enhance_process_path() };
