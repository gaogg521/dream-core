...
use dream_core_app::{AppConfig, AppServices, build_module_states, create_router_with_states};
use dream_core_office::{ConversionService, OfficeRouterState, OfficecliWatchManager, ProxyService, SnapshotService};
...
fn build_test_office_state(
    ...
    allowed_roots: Vec<std::path::PathBuf>,
    project: std::sync::Arc<dream_core_project::ProjectService>,
) -> OfficeRouterState {
    use dream_core_office::error::OfficeError;
    use dream_core_office::types::DocType;
    use dream_core_office::{ProcessHandle, ProcessSpawner};
    ...
    impl dream_core_realtime::EventBroadcaster for NoopBroadcaster {
        fn broadcast(&self, _msg: dream_core_api_types::WebSocketMessage<serde_json::Value>) {}
    }

    let spawner: Arc<dyn ProcessSpawner> = Arc::new(NoopSpawner);
    let bc: Arc<dyn dream_core_realtime::EventBroadcaster> = Arc::new(NoopBroadcaster);
