...
async fn upgrade_from_pre_user_scope_keeps_encryption_key() {
    ...
    // ── Phase 1: a genuine pre-user-scope (≤041) database ────────────────
    {
        ...
        let mut migrator: Migrator = sqlx::migrate!("../dream-core-db/migrations");
        ...
        let key = dream_core_app::derive_encryption_key("legacy-secret-0123456789");
        let enc = dream_core_common::encrypt_string("sk-upgrade-SECRET", &key).unwrap();
        sqlx::query(
    ...
    let db = dream_core_db::init_database(&db_path).await.unwrap();
    ...
    let key = dream_core_app::derive_encryption_key(&services.jwt_secret_raw);
    let repo = Arc::new(SqliteProviderRepository::new(services.database.pool().clone()));
    let svc = dream_core_system::ProviderService::new(repo, key);
    let list = svc
