...
use dream_core_extension::skill_service::{
...
async fn aionpro_services(data_dir: &Path) -> dream_core_app::AppServices {
    ...
    dream_core_app::AppServices::from_config(db, &config).await.unwrap()
}

async fn seed_user(services: &dream_core_app::AppServices, username: &str) -> String {
    use dream_core_db::IUserRepository;
    dream_core_db::SqliteUserRepository::new(services.database.pool().clone())
...
async fn same_named_skill_is_physically_isolated_per_user() {
    ...
    let dir_a = dream_core_common::user_dir_name(&user_a).unwrap();
    let dir_b = dream_core_common::user_dir_name(&user_b).unwrap();
...
async fn provisioning_first_aionpro_user_adopts_default_user_files_on_disk() {
    ...
    let adopter_dir = dream_core_common::user_dir_name(adopter).unwrap();
