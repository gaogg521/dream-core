...
async fn insert_message(
    ...
    let msg = dream_core_db::models::MessageRow {
        id: msg_id.into(),
    ...
    dream_core_db::IConversationRepository::insert_message(&repo, &user_id, &msg)
        .await
        .unwrap();
}

async fn update_conversation_workspace(services: &dream_core_app::AppServices, conv_id: &str, workspace: &str) {
    let repo = dream_core_db::SqliteConversationRepository::new(services.database.pool().clone());
    let user_id = repo.owner_user_id(conv_id).await.unwrap().unwrap();
...
async fn upsert_artifact(services: &dream_core_app::AppServices, artifact: dream_core_db::ConversationArtifactRow) {
    let repo = dream_core_db::SqliteConversationRepository::new(services.database.pool().clone());
    let user_id = repo.owner_user_id(&artifact.conversation_id).await.unwrap().unwrap();
    dream_core_db::IConversationRepository::upsert_artifact(&repo, &user_id, &artifact)
        .await
        .unwrap();
}
...
async fn t8_8_artifacts_list_and_patch_status() {
    ...
    upsert_artifact(
        &services,
        dream_core_db::ConversationArtifactRow {
