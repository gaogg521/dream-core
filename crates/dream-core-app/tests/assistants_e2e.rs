...
//! against a real `dream_core_app::create_router_with_states` instance backed by
...
impl AssistantAgentCatalogPort for TestAgentCatalog {
    async fn list_management_agents(
        &self,
        _user_id: &str,
    ) -> Result<Vec<AgentManagementRow>, dream_core_assistant::AssistantError> {
        Ok(self.rows.clone())
    }
}
...
fn test_agent_row(id: &str, backend: Option<&str>, agent_type: AgentType, name: &str) -> AgentManagementRow {
    AgentManagementRow {
        ...
        agent_source: match agent_type {
            AgentType::DreamEngine => AgentSource::Internal,
...
async fn fixture() -> Fixture {
    ...
    let marketplace_repo: Arc<dyn dream_core_db::IAssistantMarketplaceRepository> =
        Arc::new(dream_core_db::SqliteAssistantMarketplaceRepository::new(pool.clone()));
        ...
        dream_core_assistant::service::AssistantServiceDeps {
            ...
            agent_catalog: Some(Arc::new(TestAgentCatalog {
                rows: vec![
                    ...
                    test_agent_row("632f31d2", None, AgentType::DreamEngine, "Aion CLI"),
                ],
            })),
        },
        user_data_dir.clone(),
...
async fn rule_and_skill_files_are_isolated_by_current_user() {
    ...
    assert_eq!(
        std::fs::read_to_string(
            rule_root
                .join("users")
                .join(dream_core_common::user_dir_name(&user_b.id).unwrap())
                .join("shared-assistant.md")
        )
        .unwrap(),
        "rule-b"
...
async fn seed_marketplace_persona(fx: &Fixture, id: &str, name: &str, rule_content: &str) {
    let repo = dream_core_db::SqliteAssistantMarketplaceRepository::new(fx.services.database.pool().clone());
    dream_core_db::IAssistantMarketplaceRepository::upsert_many(
        &repo,
        &[dream_core_db::UpsertMarketplacePersonaParams {
            id,
            source: "workbuddy",
            name,
...
async fn avatars_of_two_users_are_physically_isolated() {
    ...
    let dir_b = dream_core_common::user_dir_name(&user_b.id).unwrap();
