...
use dream_core_app::{AppConfig, AppServices, build_module_states, create_router_with_states, derive_encryption_key};
use dream_core_common::{decrypt_string, now_ms};
use dream_core_db::{IChannelRepository, SqliteChannelRepository};
...
async fn eq19_channel_status_merges_extension_meta_for_persisted_row() {
    ...
    repo.upsert_plugin(
        &owner_user_id,
        &dream_core_db::models::ChannelPluginRow {
...
async fn extension_enablement_and_contributions_are_isolated_by_user() {
    ...
    let missing_csrf = Request::builder()
        ...
        .header("cookie", format!("dream-session={token_a}"))
...
async fn skill_batch_import_reports_partial_failures_without_rolling_back_successes() {
    ...
    let skill_repo = dream_core_db::SqliteSkillRepository::new(services.database.pool().clone());
    let alpha_row = dream_core_db::ISkillRepository::find_by_name_for_user(&skill_repo, &user.id, "sample-alpha")
        .await
        .unwrap()
    ...
    assert!(
        dream_core_db::ISkillRepository::find_by_name_for_user(&skill_repo, &user.id, "sample-beta")
            .await
            .unwrap()
            .is_none()
    );
