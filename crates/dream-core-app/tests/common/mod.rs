...
use dream_core_app::{AppConfig, AppServices, build_module_states, create_router, create_router_with_states};
use dream_core_extension::{ExternalPathsManager, SkillPaths, SkillRouterState};
use dream_core_file::FileService;
use dream_core_system::VersionCheckService;
...
pub async fn sync_skill_catalog_for_test(services: &AppServices, paths: &SkillPaths, username: &str) {
    let repo = dream_core_db::SqliteSkillRepository::new(services.database.pool().clone());
    let user = services
        .user_repo
    ...
    dream_core_extension::sync_skill_catalog_into_repo_for_user(paths, &repo, &user.id)
        .await
...
pub async fn build_app_with_noop_opener() -> (axum::Router, AppServices) {
    ...
    let services = AppServices::from_config(db, &AppConfig::default()).await.unwrap();
    let (mut states, _) = build_module_states(&services).await.expect("build module states");
    states.shell.shell_service = std::sync::Arc::new(dream_core_shell::ShellService::new(std::sync::Arc::new(
        dream_core_shell::NoopSystemOpener,
    )));
    let router = create_router_with_states(&services, states);
    (router, services)
}
...
pub async fn build_app_with_mock_agents() -> (axum::Router, AppServices) {
    ...
    let factory: std::sync::Arc<
        dyn Fn(
                dream_core_ai_agent::types::BuildTaskOptions,
            )
                -> futures_util::future::BoxFuture<'static, Result<AgentInstance, dream_core_ai_agent::AgentError>>
            + Send
            + Sync,
    ...
    let wtm: std::sync::Arc<dyn dream_core_ai_agent::IWorkerTaskManager> =
...
impl IAgentTask for NoopMockAgent {
    fn agent_type(&self) -> dream_core_common::AgentType {
        dream_core_common::AgentType::Acp
    }
    ...
    fn status(&self) -> Option<dream_core_common::ConversationStatus> {
        None
    }
    fn last_activity_at(&self) -> dream_core_common::TimestampMs {
        dream_core_common::now_ms()
    }
    fn subscribe(&self) -> tokio::sync::broadcast::Receiver<dream_core_ai_agent::AgentStreamEvent> {
        let (tx, _) = tokio::sync::broadcast::channel(1);
        tx.subscribe()
    }
    async fn send_message(
        &self,
        _data: dream_core_ai_agent::types::SendMessageData,
    ) -> Result<(), dream_core_ai_agent::AgentSendError> {
        Ok(())
    }
    async fn cancel(&self) -> Result<(), dream_core_ai_agent::AgentError> {
        Ok(())
    }
    fn kill(&self, _reason: Option<dream_core_common::AgentKillReason>) -> Result<(), dream_core_ai_agent::AgentError> {
        Ok(())
    }
}
...
impl IMockAgent for NoopMockAgent {
    async fn get_config_options(
        &self,
    ) -> Result<dream_core_api_types::GetConfigOptionsResponse, dream_core_ai_agent::AgentError> {
        Ok(dream_core_api_types::GetConfigOptionsResponse {
            config_options: vec![dream_core_api_types::AcpConfigOptionDto {
                id: "model".to_owned(),
                ...
                options: vec![dream_core_api_types::AcpConfigSelectOptionDto {
    ...
    async fn set_config_option(
        ...
        _option_id: &str,
        _value: &str,
    ) -> Result<dream_core_api_types::SetConfigOptionResponse, dream_core_ai_agent::AgentError> {
        Ok(dream_core_api_types::SetConfigOptionResponse {
            confirmation: dream_core_api_types::ConfigOptionConfirmation::Observed,
            config_options: Some(vec![dream_core_api_types::AcpConfigOptionDto {
                id: "model".to_owned(),
                name: Some("Model".to_owned()),
                label: None,
                description: None,
...
pub fn extract_csrf_token(resp: &axum::response::Response) -> Option<String> {
    resp.headers()
        ...
        .find(|s| s.starts_with("dream-csrf-token="))
        .map(|s| {
            s.strip_prefix("dream-csrf-token=")
                .unwrap()
                .split(';')
                .next()
                .unwrap()
...
pub fn json_with_token(method_str: &str, uri: &str, body: serde_json::Value, token: &str, csrf: &str) -> Request<Body> {
    Request::builder()
        ...
        .header("cookie", format!("dream-csrf-token={csrf}"))
...
pub async fn setup_and_login(
    ...
    let hash = dream_core_auth::hash_password(password).unwrap();
    if username == "admin" {
        services
            .user_repo
