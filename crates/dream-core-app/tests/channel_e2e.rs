...
use dream_core_db::models::{AssistantSessionRow, AssistantUserRow};
...
async fn get_plugins_empty() {
    ...
    assert_eq!(
        types,
        std::collections::HashSet::from(["telegram", "lark", "dingtalk", "slack", "discord", "weixin",])
    );
...
async fn pairing_approve_creates_user() {
    ...
    let repo: std::sync::Arc<dyn dream_core_db::IChannelRepository> =
        std::sync::Arc::new(dream_core_db::SqliteChannelRepository::new(pool));
    let pairing_svc = dream_core_channel::pairing::PairingService::new(repo.clone(), services.event_bus.clone());
