...
async fn start_live_app() -> LiveApp {
    start_live_app_on(dream_core_db::init_database_memory().await.unwrap(), true).await
}
...
/// it again.
async fn start_live_app_on(db: dream_core_db::Database, create_user: bool) -> LiveApp {
...
async fn run_backend_cancel(backend: &str) {
    let app = start_live_app().await;
    let ws_dir = std::env::temp_dir().join(format!("live-cancel-{backend}-{}", dream_core_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
...
async fn run_backend_usage(backend: &str, expects_window_size: bool) {
    let app = start_live_app().await;
    let ws_dir = std::env::temp_dir().join(format!("live-usage-{backend}-{}", dream_core_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
...
async fn run_backend_set_model(backend: &str) {
    let app = start_live_app().await;
    let ws_dir = std::env::temp_dir().join(format!("live-model-{backend}-{}", dream_core_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
...
async fn run_backend_session_title(backend: &str) {
    let app = start_live_app().await;
    let ws_dir = std::env::temp_dir().join(format!("live-title-{backend}-{}", dream_core_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
...
async fn run_backend_mcp_provisioning(backend: &str) {
    let app = start_live_app().await;
    let ws_dir = std::env::temp_dir().join(format!("live-mcp-{backend}-{}", dream_core_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
...
async fn run_direct_backend_team_mcp_and_runtime_env(backend: &str, agent_id: &str) {
    ...
    let app = start_live_app().await;
    let suffix = dream_core_common::now_ms();
...
async fn run_backend_permission_prompt(backend: &str) {
    let app = start_live_app().await;
    let ws_dir = std::env::temp_dir().join(format!("live-perm-{backend}-{}", dream_core_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
...
async fn conversation_for(app: &LiveApp, backend: &str, label: &str) -> String {
    let ws_dir = std::env::temp_dir().join(format!("live-{label}-{backend}-{}", dream_core_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
    let created = http_json(
        app,
        "POST",
...
async fn live_claude_ask_user_question_round_trip() {
    let app = start_live_app().await;
    let ws_dir = std::env::temp_dir().join(format!("live-ask-{}", dream_core_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
...
async fn live_codex_full_access_writes_without_approval() {
    ...
    let ws_dir = std::env::temp_dir().join(format!("live-fullaccess-{}", dream_core_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
...
async fn live_claude_workflow_natural_completion_still_finishes() {
    ...
    let _ = tracing_subscriber::fmt()
        .with_env_filter(tracing_subscriber::EnvFilter::new(
            "dream_core_ai_agent=debug,dream_core_session=info",
        ))
        .try_init();
    let app = start_live_app().await;

    let ws_dir = std::env::temp_dir().join(format!("live-wf-natural-{}", dream_core_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
...
async fn live_claude_workflow_cancel_recovers_conversation() {
    ...
    let _ = tracing_subscriber::fmt()
        .with_env_filter(tracing_subscriber::EnvFilter::new(
            "dream_core_ai_agent=debug,dream_core_session=debug",
        ))
        .try_init();
    let app = start_live_app().await;

    let ws_dir = std::env::temp_dir().join(format!("live-wf-cancel-{}", dream_core_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
...
async fn live_claude_workflow_progress_streams() {
    ...
    let ws_dir = std::env::temp_dir().join(format!("live-wf-progress-{}", dream_core_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
...
async fn live_codex_background_work_probe() {
    let _ = tracing_subscriber::fmt()
        .with_env_filter(tracing_subscriber::EnvFilter::new(
            "dream_core_ai_agent=debug,dream_core_session=debug,dream_core_conversation=info",
        ))
        .try_init();
    let app = start_live_app().await;

    let ws_dir = std::env::temp_dir().join(format!("live-codex-bg-{}", dream_core_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
