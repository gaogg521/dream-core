...
async fn user_id_for_username(services: &dream_core_app::AppServices, username: &str) -> String {
    services
        .user_repo
...
async fn management_list_includes_missing_custom_agents() {
    ...
    repo.upsert_for_user(
        ...
        &UpsertAgentMetadataParams {
            ...
            agent_type: "acp",
            agent_source: "custom",
            agent_source_info: Some(r#"{"binary_name":"one-missing-agent-binary"}"#),
            enabled: true,
            command: Some("one-missing-agent-binary"),
            args: Some("[]"),
            env: Some("[]"),
