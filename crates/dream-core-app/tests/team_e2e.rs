...
use dream_core_db::{IConversationRepository, MessagePageDirection, MessagePageParams};
...
async fn ensure_default_team_agent_installed(services: &dream_core_app::AppServices) {
...
async fn mark_claude_backend_team_mcp_stdio_capable(services: &dream_core_app::AppServices) {
...
async fn tc3b_create_team_writes_legacy_extra_shape() {
    ...
    let repo = dream_core_db::SqliteConversationRepository::new(services.database.pool().clone());
...
async fn tc6c_create_team_rejects_missing_workspace_path() {
    ...
    let missing_workspace =
        std::env::temp_dir().join(format!("aionui-team-missing-{}", dream_core_common::generate_short_id()));
...
async fn es1c_team_conversations_carry_assistant_bound_mcp_snapshot() {
    ...
    let now = dream_core_common::now_ms() as i64;
...
async fn context_reset_returns_structured_success_and_projects_a_semantic_notice() {
    ...
    let conversation_repo = dream_core_db::SqliteConversationRepository::new(services.database.pool().clone());
...
async fn conversation_extra(services: &dream_core_app::AppServices, conversation_id: &str) -> Value {
    let repo = dream_core_db::SqliteConversationRepository::new(services.database.pool().clone());
