...
//! auth routes) via `dream_core_app::create_router`, covering test-plan items
...
fn extract_session_token(resp: &axum::response::Response) -> Option<String> {
    resp.headers()
        ...
        .iter()
        .filter_map(|v| v.to_str().ok())
        .find(|s| s.starts_with("dream-session="))
        .and_then(|s| {
            let value = s.strip_prefix("dream-session=")?.split(';').next()?.to_owned();
...
async fn t12_3_session_cookie_attributes() {
    ...
    let hash = dream_core_auth::hash_password("StrongP@ss1").unwrap();
...
async fn t13_1_authorization_header_takes_priority() {
    ...
    let req = Request::builder()
        ...
        .uri("/api/auth/user")
        .header("authorization", format!("Bearer {token}"))
        .header("cookie", "dream-session=invalid_token")
        .body(Body::empty())
        .unwrap();
    let resp = app.oneshot(req).await.unwrap();
