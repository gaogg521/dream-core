...
use dream_core_ai_agent::{AgentError, AgentStreamEvent, IWorkerTaskManager};
...
async fn build_app_with_mock_tasks() -> (axum::Router, dream_core_app::AppServices, Arc<MockTaskManager>) {
    ...
    let router = dream_core_app::create_router(&services).await.expect("build router");
...
async fn upsert_visible_agent_metadata(services: &dream_core_app::AppServices, id: &str, agent_type: &str) {
    services
        .agent_registry
        .repo_handle()
