...
use dream_core_common::{PaginatedResult, ProviderWithModel, TimestampMs, now_ms};
use dream_core_conversation::ConversationService;
use dream_core_cron::{CronRouterState, cron_routes};
use dream_core_db::{
...
use dream_core_cron::scheduler::CronScheduler;
use dream_core_cron::service::{CronService, CronServiceDeps};
use dream_core_cron::types::CronAgentConfig;
use dream_core_cron::types::JobStatus;
...
async fn seed_sqlite_conversations(db: &dream_core_db::Database, conversation_ids: &[&str]) {
    sqlx::query(
...
impl dream_core_ai_agent::task_manager::IWorkerTaskManager for StubTaskManager {
...
struct StubConvRepo {
    ...
    artifacts: Mutex<Vec<dream_core_db::ConversationArtifactRow>>,
    rows: Mutex<HashMap<String, dream_core_db::models::ConversationRow>>,
    assistant_snapshots: Mutex<HashMap<String, ConversationAssistantSnapshotRow>>,
    update_failures: Mutex<Vec<String>>,
    sqlite_pool: dream_core_db::SqlitePool,
}

impl StubConvRepo {
    fn new(sqlite_pool: dream_core_db::SqlitePool) -> Self {
        Self {
    ...
    fn upsert_artifact_row(&self, artifact: dream_core_db::ConversationArtifactRow) {
    ...
    fn artifacts(&self) -> Vec<dream_core_db::ConversationArtifactRow> {
    ...
    fn set_conversation_extra(&self, conversation_id: &str, extra: serde_json::Value) {
        ...
        let row = rows
            ...
            .or_insert_with(|| dream_core_db::models::ConversationRow {
                id: conversation_id.to_owned(),
                user_id: "u1".into(),
                name: "stub".into(),
                r#type: "default".into(),
...
impl IConversationRepository for StubConvRepo {
    ...
    async fn owner_user_id(&self, id: &str) -> Result<Option<String>, dream_core_db::DbError> {
    ...
    async fn get_assistant_snapshot(
        ...
        _user_id: &str,
        conversation_id: &str,
    ) -> Result<Option<ConversationAssistantSnapshotRow>, dream_core_db::DbError> {
    ...
    async fn create(&self, row: &dream_core_db::models::ConversationRow) -> Result<(), dream_core_db::DbError> {
    ...
    async fn update(
        ...
        if self.update_failures.lock().unwrap().iter().any(|item| item == id) {
            return Err(dream_core_db::DbError::Init(format!("forced update failure for {id}")));
        }
    ...
    async fn list_artifacts(
        ...
        _user_id: &str,
        conversation_id: &str,
    ) -> Result<Vec<dream_core_db::ConversationArtifactRow>, dream_core_db::DbError> {
        Ok(self
            .artifacts
            .lock()
            .unwrap()
    ...
    async fn get_artifact(
        ...
        conversation_id: &str,
        artifact_id: &str,
    ) -> Result<Option<dream_core_db::ConversationArtifactRow>, dream_core_db::DbError> {
        Ok(self
            .artifacts
            .lock()
            .unwrap()
    ...
    async fn upsert_artifact(
        &self,
        _user_id: &str,
        artifact: &dream_core_db::ConversationArtifactRow,
    ) -> Result<dream_core_db::ConversationArtifactRow, dream_core_db::DbError> {
...
async fn setup_with_conv_runtime_and_agent_metadata() -> (
    ...
    Arc<dyn dream_core_db::ISkillRepository>,
) {
    ...
    let task_manager: Arc<dyn dream_core_ai_agent::task_manager::IWorkerTaskManager> = Arc::new(StubTaskManager);
    ...
    let skill_repo: Arc<dyn dream_core_db::ISkillRepository> = Arc::new(SqliteSkillRepository::new(pool.clone()));
...
async fn setup_with_skill_repo() -> (CronService, Arc<dyn dream_core_db::ISkillRepository>) {
...
fn make_create_req(name: &str, schedule: CronScheduleDto) -> CreateCronJobRequest {
    CreateCronJobRequest {
        ...
        execution_mode: None,
        queue_enabled: false,
        agent_config: Some(dream_core_api_types::CronAgentConfigWriteDto {
...
async fn create_job_requires_assistant_id_for_new_jobs() {
    ...
    assert!(matches!(err, dream_core_cron::error::CronError::InvalidAgentConfig(_)));
...
async fn cj5_get_nonexistent_job() {
    ...
    assert!(matches!(err, dream_core_cron::error::CronError::JobNotFound(_)));
}
...
async fn update_existing_conversation_job_rejects_agent_config_changes() {
    ...
    assert!(
        matches!(err, dream_core_cron::error::CronError::InvalidAgentConfig(message) if message.contains("ongoing conversation"))
    );
}
...
async fn cj9_update_schedule_type() {
    ...
    assert!(matches!(
        updated.schedule,
        dream_core_cron::types::CronSchedule::Cron { .. }
    ));
...
async fn cj12_delete_nonexistent() {
    ...
    assert!(matches!(
        err,
        dream_core_cron::error::CronError::Database(dream_core_db::DbError::NotFound(_))
    ));
}
...
async fn sk1_1_save_skill_marks_related_skill_suggest_artifacts_saved() {
    ...
    conv_repo.upsert_artifact_row(dream_core_db::ConversationArtifactRow {
...
async fn sk4_save_empty_skill() {
    ...
    let err = svc
        ...
        .await
        .unwrap_err();
    assert!(matches!(err, dream_core_cron::error::CronError::InvalidSkillContent(_)));
}
...
async fn sc7_every_zero_interval() {
    ...
    let err = svc.add_job("u1", req).await.unwrap_err();
    assert!(matches!(err, dream_core_cron::error::CronError::InvalidSchedule(_)));
}
...
async fn oc1_rejects_lazy_existing_jobs() {
    ...
    let err = svc.add_job("u1", req).await.unwrap_err();
    assert!(matches!(err, dream_core_cron::error::CronError::Conversation(_)));
}
...
async fn oc2b_rejects_existing_jobs_with_cross_user_conversation_code() {
    ...
    .unwrap();
    conv_repo
        .create(&dream_core_db::models::ConversationRow {
    ...
    assert!(matches!(
        err,
        dream_core_cron::error::CronError::CrossAccountReference(message)
...
async fn create_for_conversation_helper_fails_when_conversation_binding_fails() {
    ...
    assert!(matches!(err, dream_core_cron::error::CronError::Database(_)));
...
async fn sr1_system_resume_missed_job() {
    ...
    let params = dream_core_db::UpdateCronJobParams {
