...
use dream_core_extension::hub::{HubIndexManager, HubInstaller};
use dream_core_extension::registry::ExtensionRegistry;
use dream_core_extension::state::ExtensionStateStore;
use dream_core_extension::types::HubExtensionStatus;
...
async fn hm5_check_updates_detects_newer_version() {
    ...
    let scan_paths = vec![dream_core_extension::loader::ScanPath {
        path: h.hub_dir.path().to_path_buf(),
        source: dream_core_extension::types::ExtensionSource::Env,
    }];
