...
use dream_core_extension::classifier::{AssistantClassifier, AssistantRuleDispatcher};
use dream_core_extension::error::ExtensionError;
...
async fn router_with_dispatcher(dispatcher: Arc<FakeDispatcher>) -> axum::Router {
    ...
    let skill_repo = Arc::new(dream_core_db::SqliteSkillRepository::new(db.pool().clone()));
    std::mem::forget(tmp);
