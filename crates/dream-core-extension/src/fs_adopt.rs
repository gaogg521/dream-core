...
pub async fn adopt_user_filesystem(
    ...
    let Ok(adopter_dir) = dream_core_common::user_dir_name(adopter_user_id) else {
...
mod tests {
    use super::*;
    use dream_core_db::{SqliteSkillRepository, UpsertSkillParams, init_database_memory};
    ...
    async fn seed_user(db: &dream_core_db::Database) -> String {
        use dream_core_db::IUserRepository;
        dream_core_db::SqliteUserRepository::new(db.pool().clone())
