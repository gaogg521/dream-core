...
use dream_core_db::{CreateSkillImportRecordParams, ISkillRepository, SkillRow, UpsertSkillParams};
...
pub async fn import_skills(paths: &SkillPaths, source_path: &Path) -> Result<SkillImportOutcome, ExtensionError> {
    let operation_id = dream_core_common::generate_prefixed_id("skill_import_op");
...
async fn import_skills_from_zip(paths: &SkillPaths, archive_path: &Path) -> Result<SkillImportOutcome, ExtensionError> {
    ...
    let result = async {
        ...
        import_skill_dirs_batch(
            paths,
            skill_dirs,
            dream_core_common::generate_prefixed_id("skill_import_op"),
        )
        .await
    }
    .await;
...
mod tests {
    use super::*;
    use dream_core_db::{ISkillRepository, SqliteSkillRepository, init_database_memory};
    use std::io::Write;
