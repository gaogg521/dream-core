...
use dream_core_file::{FileError, path_safety::validate_path_with_extra_root};
...
fn refresh_resolve_error(err: dream_core_project::ProjectError) -> ApiError {
    let code = err.code();
...
) -> Result<Json<ApiResponse<dream_core_api_types::DocumentConversionResponse>>, ApiError> {
    let Json(req) = body.map_err(ApiError::from)?;
