...
use dream_core_common::{McpServerStatus, McpSource, TimestampMs};
...
impl From<dream_core_common::SchemaIncompatibility> for McpToolIncompatibility {
    fn from(value: dream_core_common::SchemaIncompatibility) -> Self {
        Self {
            path: value.path,
            key: value.key,
...
impl McpToolResponse {
    ...
    pub fn new(name: String, description: Option<String>, input_schema: Option<serde_json::Value>) -> Self {
        let incompatibilities = dream_core_common::validate_tool(&name, input_schema.as_ref())
            .into_iter()
