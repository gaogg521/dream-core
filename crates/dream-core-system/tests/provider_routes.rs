...
fn build_state(db: &dream_core_db::Database) -> SystemRouterState {
...
async fn setup() -> (axum::Router, dream_core_db::Database) {
...
async fn create_one(db: &dream_core_db::Database) -> (serde_json::Value, String) {
...
fn webui_get_request_for_user(user_id: &str, uri: &str) -> Request<Body> {
    ...
    req.headers_mut().insert(
        dream_core_auth::WEBUI_PROXY_HEADER,
        dream_core_auth::WEBUI_PROXY_VALUE.parse().unwrap(),
    );
    req
}
