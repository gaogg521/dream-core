...
fn spawn_keep_awake_process() -> Result<tokio::process::Child, SystemError> {
    let mut command = dream_core_runtime::Builder::clean_cli("caffeinate");
...
#[cfg(target_os = "linux")]
fn spawn_keep_awake_process() -> Result<tokio::process::Child, SystemError> {
    let mut command = dream_core_runtime::Builder::clean_cli("systemd-inhibit");
    command
        .args([
            "--what=sleep",
            "--why=One Work scheduled tasks keep-awake is enabled",
            "--mode=block",
            "sleep",
            "infinity",
        ])
