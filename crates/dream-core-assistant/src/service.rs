...
use dream_core_extension::{AssistantClassifier, AssistantRuleDispatcher, ExtensionError};
...
fn assistant_error_is_busy(error: &AssistantError) -> bool {
    matches!(error, AssistantError::Internal(message) if dream_core_db::message_indicates_busy(message))
}
...
fn assistant_error_is_unique(error: &AssistantError) -> bool {
    match error {
        ...
        AssistantError::Internal(message) => dream_core_db::message_indicates_unique_violation(message),
        _ => false,
    }
}
    ...
    pub async fn import_for_user(
        ...
        for entry in req.assistants {
            ...
            match self.repo.create_for_user(user_id, &params).await {
                ...
                Err(dream_core_db::DbError::Conflict(_)) => {
...
impl AssistantService {
    ...
    fn definition_to_detail_response(
        ...
        definition: &AssistantDefinitionRow,
        state: Option<&AssistantOverlayRow>,
        preference: Option<&dream_core_db::AssistantPreferenceRow>,
        rules_content: &str,
        projection: &AssistantRuntimeProjection,
    ) -> Result<AssistantDetailResponse, AssistantError> {
        let builtin_default = self.builtin_listing_default(definition);
...
mod tests {
    ...
    struct Fixture {
        ...
        override_repo: Arc<dyn IAssistantOverrideRepository>,
        provider_repo: Arc<dyn IProviderRepository>,
        agent_rows: Arc<Mutex<Vec<dream_core_api_types::AgentManagementRow>>>,
        _tmp: TempDir,
        _db: dream_core_db::Database,
    }

    #[derive(Clone, Default)]
    struct StubAgentCatalog {
        rows: Arc<Mutex<Vec<dream_core_api_types::AgentManagementRow>>>,
    }
    ...
    async fn create_test_user(db: &dream_core_db::Database, username: &str) -> String {
        let repo = dream_core_db::SqliteUserRepository::new(db.pool().clone());
    ...
    fn mk_agent_row(
        id: &str,
        backend: &str,
        status: dream_core_api_types::AgentManagementStatus,
    ) -> dream_core_api_types::AgentManagementRow {
        dream_core_api_types::AgentManagementRow {
            id: id.into(),
            ...
            behavior_policy: dream_core_api_types::BehaviorPolicy {
                supports_team: true,
                ..Default::default()
            },
            yolo_id: None,
            ...
            team_capable: true,
            status,
            last_check_status: Some(dream_core_api_types::AgentSnapshotCheckStatus::Online),
            last_check_kind: Some(dream_core_api_types::AgentSnapshotCheckKind::Manual),
    ...
    fn mk_uninstalled_agent_row(id: &str, backend: &str) -> dream_core_api_types::AgentManagementRow {
        let mut row = mk_agent_row(id, backend, dream_core_api_types::AgentManagementStatus::Unchecked);
        row.installed = false;
    ...
    async fn bootstrap_materializes_generated_assistant_from_available_agent() {
        ...
        assert_eq!(bare.agent_status, dream_core_api_types::AgentManagementStatus::Online);
    ...
    async fn bootstrap_materializes_generated_assistant_from_offline_agent() {
        ...
        let mut offline_row = mk_agent_row(
            "agent-cursor",
            "cursor",
            dream_core_api_types::AgentManagementStatus::Offline,
        );
        offline_row.last_check_status = Some(dream_core_api_types::AgentSnapshotCheckStatus::Offline);
        ...
        assert_eq!(bare.source, AssistantSource::Generated);
        assert_eq!(bare.agent_id, "agent-cursor");
        assert_eq!(bare.agent_status, dream_core_api_types::AgentManagementStatus::Offline);
    }
    ...
    async fn list_hides_existing_generated_assistant_when_agent_is_uninstalled_until_installed() {
        ...
        uninstalled_row.status = dream_core_api_types::AgentManagementStatus::Offline;
        let fx = fixture_with_options(FixtureOpts {
...

        {
            ...
            rows[0].status = dream_core_api_types::AgentManagementStatus::Unchecked;
        }
    ...
    async fn bootstrap_materializes_generated_assistant_from_available_custom_agent() {
        ...
        custom_row.agent_source = dream_core_api_types::AgentSource::Custom;
    ...
    async fn bootstrap_falls_back_to_agent_type_when_backend_is_empty() {
        ...
        agent_row.agent_type = dream_core_common::AgentType::DreamEngine;
